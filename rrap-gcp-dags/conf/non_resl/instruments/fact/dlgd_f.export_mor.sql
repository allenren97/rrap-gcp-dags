...
CASE 
    WHEN 
            b.MTH_TM_ID >= 16996 
        AND seg.LGD_BASEL_SEG_NUM NOT IN (13,14) 
        AND A.PIT_STAT_CD = 'CUR'
        and c.SCRTY_TP_DESC in  ('UNINSURED','INSURED') 
        AND (B.INTR_ADJ_DT>='2016-11-01' OR B.LAST_RNEW_DT>='2016-11-01') 
    THEN 'Y' 
    WHEN
            b.MTH_TM_ID < 16996  
        AND seg.LGD_BASEL_SEG_NUM NOT IN (13,14) 
        AND A.PIT_STAT_CD = 'CUR'
        AND c.SCRTY_TP_DESC='UNINSURED' 
        AND (B.INTR_ADJ_DT>='2016-11-01' OR B.LAST_RNEW_DT>='2016-11-01') 
    THEN 'Y'
    ELSE 'N' 
END AS DLGD_F 

WITH get_pop as (
    ...
    mapping AS(
        SELECT
            ...
            mod.SRC_SYS_CD
        FROM 
            {{upstream_asset[2]}} AS parm
            LEFT JOIN {{upstream_asset[3]}} AS seg ON parm.basel_seg_id = seg.basel_seg_id
            LEFT JOIN {{upstream_asset[4]}} AS MOD ON MOD.basel_model_id = parm.basel_model_id
        WHERE
    ...
    seg_num AS (
        ...
        LEFT JOIN {{upstream_asset[6]}} lgd_seg ON
        ...
        LEFT JOIN {{upstream_asset[7]}} nm ON
        ...
        LEFT JOIN {{upstream_asset[3]}} AS seg ON 
    ...
    SELECT
        ...
        CASE
            ...
            ELSE mapping.UNADJUSTED_RPTG_RTO
        END AS PMI_LGD_UNADJUSTED_RPTG_RTO
    FROM get_pop acct
    LEFT JOIN seg_num AS main ON
        acct.BASEL_ACCT_ID = main.BASEL_ACCT_ID
    LEFT JOIN {{upstream_asset[1]}} prd_tp ON

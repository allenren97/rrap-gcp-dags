...
LEFT JOIN {{upstream_asset[0]}} dim
    ON dim.SRC_APP_CD = 'TNG-MOR'
    AND dim.SRC_SYS_DEL_F != 'Y'
    AND TRIM(tng.ACCOUNT_ID) = TRIM(dim.SRC_APP_ID)
LEFT JOIN {{upstream_asset[1]}} dlgd
    ON dlgd.BASEL_ACCT_ID = dim.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[3]}} lgd_rto
    on lgd_rto.BASEL_ACCT_ID = dim.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[4]}} lgd_flr
    on lgd_flr.BASEL_ACCT_ID = dim.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[2]}} dlgd_flr
    on dlgd_flr.BASEL_ACCT_ID = dim.BASEL_ACCT_ID

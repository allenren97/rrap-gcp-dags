    ...
    IF SRC_SYS_CD = 'MOR'
      THEN CASE 
        IF STEP_PLNT_AGRMNT_NUM IS NOT NULL
          ...
          END
        ELSE CASE
          IF MODEL = 'mor_lgdd' AND LGD_BASEL_SEG_NUM IN (1, 3, 5) THEN 1
          IF MODEL = 'mor_lgdd' AND LGD_BASEL_SEG_NUM IN (2, 4, 6) THEN 2
          IF MODEL = 'mor_lgdnd' AND LGD_BASEL_SEG_NUM IN (1, 2, 3) THEN 1
          ELSE LGD_BASEL_SEG_NUM
        END
      END
    ELSE LGD_BASEL_SEG_NUM

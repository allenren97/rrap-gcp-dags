...
LEFT JOIN {{upstream_asset[1]}} dlgd
    ON dlgd.BASEL_ACCT_ID = ks.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[3]}} lgd_rto
    on lgd_rto.BASEL_ACCT_ID = ks.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[4]}} lgd_flr
    on lgd_flr.BASEL_ACCT_ID = ks.BASEL_ACCT_ID
...
LEFT JOIN {{upstream_asset[2]}} dlgd_flr
    on dlgd_flr.BASEL_ACCT_ID = ks.BASEL_ACCT_ID

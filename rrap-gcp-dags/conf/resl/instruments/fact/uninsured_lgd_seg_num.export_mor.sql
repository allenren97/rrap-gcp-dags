...
left join (
select basel_acct_id, lgd_model_nm,lgd_model_id from {{upstream_asset[2]}}  
where obsn_dt='{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}' 
...
and ((lgd_model_id) = 'mor_lgdd') and lgd_basel_seg_num IN (3,5) THEN 1
WHEN (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'mor_lgdd') and lgd_basel_seg_num IN (4,6) THEN 2
WHEN (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'mor_lgdnd') and lgd_basel_seg_num IN (2,3) THEN 1

when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdd') and lgd_basel_seg_num IN (4,7) THEN 1 
when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdd') and lgd_basel_seg_num IN (5,8) THEN 2
when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdd') and lgd_basel_seg_num IN (6,9) THEN 3
when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdnd') and lgd_basel_seg_num IN (4,7) THEN 1 
when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdnd') and lgd_basel_seg_num IN (5,8) THEN 2
when (trim(CCAR_BASEL_PRD_TP_NM) like 'GENW%' OR trim(CCAR_BASEL_PRD_TP_NM) LIKE 'GUAR%')
and ((lgd_model_id) = 'step_mix_mor_lgdnd') and lgd_basel_seg_num IN (6,9) THEN 3

...
UPSTREAM_ASSET = [
    f"models.{MODEL.upper()}_TRANSACTOR_SCORE",
    f"models.{MODEL.upper()}_DELINQUENT_SCORE",
    f"models.{MODEL.upper()}_REVOLVER_SCORE",
    "features.HELOC_F",
...
def export_acct_list(
    ...
    sql=r"""
        WITH
            ...
            CC_TRANSACTOR_F AS (
                select * from features.CC_TRANSACTOR_F where obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
            )
        SELECT
            main.*,
            ...
            CC_TRANSACTOR_F.CC_TRANSACTOR_F
        FROM
            rvl AS main
            ...
            LEFT JOIN CC_TRANSACTOR_F ON main.BASEL_ACCT_ID = CC_TRANSACTOR_F.BASEL_ACCT_ID
        WHERE
            TRIM(BASEL_PRD_CD) = 'CC'
            AND TRIM(HELOC_F) = 'N'
            AND TRIM(CONSM_PRD_TREATMNT_CD) = 'A'
            AND TRIM(TREATMENT_F) = 'A'
            AND (TRIM(PIT_STATUS_CROSS_DEFAULT_ORIG) = 'CUR' AND TRIM(COALESCE(CC_TRANSACTOR_F, '')) != ''
                OR TRIM(PIT_STATUS_CROSS_DEFAULT_ORIG) = 'DEF')
    """,
):
    pass
...
def export_segment_input(
    ...
    sql=f"""
        select b.*, a.SCORE
        from '{{{{ task_instance.xcom_pull(task_ids="segmented__cc_pd.export_acct_list", key="parquet") }}}}' b
        left join ( 
            (select BASEL_ACCT_ID, VAR_SCORE as SCORE from {UPSTREAM_ASSET[0]} where trim(VAR_NAME) = 'SCORE' and obsn_dt = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and trim(upper(model)) = trim(upper('cc_pd_transactor')) and trim(upper(stream)) = trim(upper('{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}')))
            UNION ALL
            (select BASEL_ACCT_ID, VAR_SCORE as SCORE from {UPSTREAM_ASSET[1]} where trim(VAR_NAME) = 'SCORE' and obsn_dt = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and trim(upper(model)) = trim(upper('cc_pd_delinquent')) and trim(upper(stream)) = trim(upper('{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}')))
            UNION ALL
            (select BASEL_ACCT_ID, VAR_SCORE as SCORE from {UPSTREAM_ASSET[2]} where trim(VAR_NAME) = 'SCORE' and obsn_dt = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and trim(upper(model)) = trim(upper('cc_pd_revolver')) and trim(upper(stream)) = trim(upper('{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}'))) 
        ) a on a.BASEL_ACCT_ID = b.BASEL_ACCT_ID
    """,
):
    """The query for cc pd pulls data from three upstream assets, ccpd revolver, delinquent and transactor as they are separate scoring portfolios."""
    pass
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__cc_pd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__cc_pd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

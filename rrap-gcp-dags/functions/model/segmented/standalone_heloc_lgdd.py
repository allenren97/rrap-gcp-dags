...
WITH
    ...
    STEP_STANDALONE_MONTH_DEF_SINCE_LAST_DEF AS (
        SELECT * FROM features.STEP_STANDALONE_MONTH_DEF_SINCE_LAST_DEF WHERE obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
        -- AND src_sys_cd = 'KS'
    ),
    CR_LMT_AMT AS (
...
FROM
    rvl AS main
    LEFT JOIN STEP_STANDALONE_MONTH_DEF_SINCE_LAST_DEF ON main.BASEL_ACCT_ID = STEP_STANDALONE_MONTH_DEF_SINCE_LAST_DEF.BASEL_ACCT_ID
...
WHERE
    TRIM(PIT_STATUS.PIT_STATUS_ACCOUNT) = 'DEF'
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__standalone_heloc_lgdd.export_acct_list", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__standalone_heloc_lgdd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

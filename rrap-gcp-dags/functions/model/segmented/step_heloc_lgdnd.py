...
def export_acct_list(
    ...
    sql=r"""
        ...
        WHERE
            TRIM(PIT_STATUS.PIT_STATUS_STEP) = 'CUR'
            AND TRIM(TREATMENT_F.TREATMENT_F) = 'A'
            AND UPPER(TRIM(STEP_SUB_PORT.STEP_SUB_PORT)) = 'STEP_HELOC'
""",
):
    pass
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__step_heloc_lgdnd.export_acct_list", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__step_heloc_lgdnd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

...
def export_segment_input(
    ...
    sql=f"""
        SELECT
            ...
            a.SCORE
        FROM
            '{{{{ task_instance.xcom_pull(task_ids="segmented__step_mix_mor_lgdnd.export_acct_list", key="parquet") }}}}' b
            LEFT JOIN (
                SELECT
                    BASEL_ACCT_ID,
                    VAR_SCORE AS SCORE
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__step_mix_mor_lgdnd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__step_mix_mor_lgdnd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

...
AND
	PIT_STATUS_CROSS_DEFAULT_ORIG not in ('CUR', 'CHG')
    """,
):
    pass
...
def export_segment_input(
    ...
    sql=f"""
        ...
        from '{{{{ task_instance.xcom_pull(task_ids="segmented__sslb_lgdd.export_acct_list", key="parquet") }}}}' b
    """,
):
    pass
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids=f"segmented__sslb_lgdd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__sslb_lgdd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

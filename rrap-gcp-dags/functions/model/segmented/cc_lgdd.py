...
def export_acct_list(
    ...
    sql=r"""
        ...
        FROM
            ...
            LEFT JOIN MONTH_DEF_24M AS MONTH_DEF_24M ON main.BASEL_ACCT_ID = MONTH_DEF_24M.BASEL_ACCT_ID
...
def export_segment_input(
    ...
    sql=f"""
        ...
        from '{{{{ task_instance.xcom_pull(task_ids="segmented__cc_lgdd.export_acct_list", key="parquet") }}}}' b
    """,
):
    pass
...
def get_segment():
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__cc_lgdd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__cc_lgdd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

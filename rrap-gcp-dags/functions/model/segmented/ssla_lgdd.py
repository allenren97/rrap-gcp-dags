...
INNER JOIN {UPSTREAM_ASSET[6]} g ON
    a.BASEL_ACCT_ID = g.BASEL_ACCT_ID
    AND b.OBSN_DT = g.OBSN_DT
WHERE
    a.MTH_TM_ID = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="mth_tm_id") }}}}'
AND
    b.PIT_STATUS_CROSS_DEFAULT_ORIG NOT IN ('CUR', 'CHG')
    """,
):
    pass
...
def export_segment_input(
    ...
    sql=f"""
        ...
        from '{{{{ task_instance.xcom_pull(task_ids="segmented__ssla_lgdd.export_acct_list", key="parquet") }}}}' b
    """,
):
    pass
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids=f"segmented__ssla_lgdd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__ssla_lgdd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass
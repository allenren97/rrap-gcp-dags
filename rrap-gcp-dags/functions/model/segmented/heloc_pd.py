    ...
    sql=r"""
        WITH
            ...
            CONSM_SCORECRD_EXCLSN_F as (
                select CONSM_SCORECRD_EXCLSN_F,BASEL_ACCT_ID from features.CONSM_SCORECRD_EXCLSN_F where obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
            ),
            HELOC_F as (
                select HELOC_F,BASEL_ACCT_ID from features.HELOC_F where obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
            ),
            PIT_STATUS as (
                select PIT_STATUS_CROSS_DEFAULT_ORIG,BASEL_ACCT_ID from features.PIT_STATUS_CROSS_DEFAULT_ORIG where obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
            ),
            CONSM_PRD_TREATMNT_CD  as (
                select CONSM_PRD_TREATMNT_CD,BASEL_ACCT_ID from features.CONSM_PRD_TREATMNT_CD where obsn_dt = '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}'
            )
        SELECT
            main.*,
        ...
        WHERE
            ...
            AND TRIM(PIT_STATUS.PIT_STATUS_CROSS_DEFAULT_ORIG) IN ('DEF', 'CUR')
...
def export_segment_input(
    ...
    sql=f"""
        select b.*, a.SCORE
        from '{{{{ task_instance.xcom_pull(task_ids="segmented__heloc_pd.export_acct_list", key="parquet") }}}}' b
...
def get_segment(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="segmented__heloc_pd.export_segment_input", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="segmented__heloc_pd.get_segment", key="parquet") }}}}'
        )
    """,
):
    pass

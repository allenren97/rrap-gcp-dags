...
WHERE
    CONSM_SCORECRD_EXCLSN_F.CONSM_SCORECRD_EXCLSN_F = 'N'
    AND (HELOC_F.HELOC_F) = 'Y'
...
def get_score(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids="scored__heloc_lgdd.export_acct_dv", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            ...
            from '{{{{ task_instance.xcom_pull(task_ids="scored__heloc_lgdd.get_score", key="parquet") }}}}'
        )
    """,
):
    pass

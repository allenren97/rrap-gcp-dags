...
def export_acct_dv(
    ...
    sql=_get_input_sql(
        ...
        r"""{{ task_instance.xcom_pull(task_ids="scored__cc_lgdnd.export_acct_list", key="parquet") }}""",
    ),
):
    pass
...
def get_score(pool="duckdb_pool", pool_slots=16):
    ...
    input_file = context["ti"].xcom_pull(
        task_ids=f"scored__cc_lgdnd.export_acct_dv", key="parquet"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT, '{MODEL}' as MODEL, '{{{{ dag.dag_id.rsplit("_",1)[0].upper() }}}}' as STREAM from '{{{{ task_instance.xcom_pull(task_ids="scored__cc_lgdnd.get_score", key="parquet") }}}}'
        )
    """,
):
    pass

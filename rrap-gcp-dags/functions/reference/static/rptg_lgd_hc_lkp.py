...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__rptg_lgd_hc_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE UPPER(TRIM(COLLATERAL_TYPE)) IN (
        SELECT UPPER(TRIM(COLLATERAL_TYPE)) FROM '{{{{ task_instance.xcom_pull(task_ids="static__rptg_lgd_hc_lkp.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__rptg_lgd_hc_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET COLLATERAL_TYPE = TRIM(COLLATERAL_TYPE)
    """
):
    pass

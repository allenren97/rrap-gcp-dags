...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__rptg_prd_lkp_mor.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(PRODUCT_ID) IN (
        SELECT TRIM(PRODUCT_ID) FROM '{{{{ task_instance.xcom_pull(task_ids="static__rptg_prd_lkp_mor.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__rptg_prd_lkp_mor.branch_decide", key="csv") }}}}')
    )
    """
):
    pass

...
DOWNSTREAM_ASSET = f"reference.DLQNT_DIM"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
}
INPUT_PATH = 'DLQNT_DIM.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__dlqnt_dim.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE DLQNT_ID IN (
        SELECT DLQNT_ID FROM '{{{{ task_instance.xcom_pull(task_ids="static__dlqnt_dim.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM '{{{{ task_instance.xcom_pull(task_ids="static__dlqnt_dim.branch_decide", key="csv") }}}}'
    )
    """
):
    pass

...
DOWNSTREAM_ASSET = f"reference.PSNL_RPTG_PRD_LKP"
DEPENDENCIES = {
    'branch_decide': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
    'duckdb_load': ['duckdb_postprocess_trim'],
}
INPUT_PATH = 'PSNL_RPTG_PRD_LKP.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__psnl_rptg_prd_lkp.duckdb_delete"]
...
def duckdb_delete(
    ...
    WHERE TRIM(PRD_ID) IN (
        SELECT TRIM(PRD_ID) FROM '{{{{ task_instance.xcom_pull(task_ids="static__psnl_rptg_prd_lkp.branch_decide", key="csv") }}}}'
    ) 
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM '{{{{ task_instance.xcom_pull(task_ids="static__psnl_rptg_prd_lkp.branch_decide", key="csv") }}}}'
    )
    """
):
    pass

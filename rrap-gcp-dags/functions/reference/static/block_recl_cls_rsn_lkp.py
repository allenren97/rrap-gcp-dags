...
DOWNSTREAM_ASSET = f"reference.BLOCK_RECL_CLS_RSN_LKP"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
    'duckdb_load': ['duckdb_postprocess_trim'],
}
INPUT_PATH = 'BLOCK_RECL_CLS_RSN_LKP.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__block_recl_cls_rsn_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(BLOCK_RECL_CD) || '_' || TRIM(CLS_RSN_CD) IN (
        SELECT TRIM(BLOCK_RECL_CD) || '_' || TRIM(CLS_RSN_CD) FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__block_recl_cls_rsn_lkp.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__block_recl_cls_rsn_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    UPDATE {DOWNSTREAM_ASSET}
    SET 
        BLOCK_RECL_CD = TRIM(BLOCK_RECL_CD),
        CLS_RSN_CD = TRIM(CLS_RSN_CD)
    """
):
    pass
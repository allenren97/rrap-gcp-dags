...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__chrg_off_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(CHRG_OFF_CD) IN (
        SELECT TRIM(CHRG_OFF_CD) FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__chrg_off_lkp.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__chrg_off_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET CHRG_OFF_CD = TRIM(CHRG_OFF_CD)
    """
):
    pass

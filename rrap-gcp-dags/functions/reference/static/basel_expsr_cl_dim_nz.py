...
DOWNSTREAM_ASSET = f"reference.BASEL_EXPSR_CL_DIM_NZ"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
}
INPUT_PATH = 'BASEL_EXPSR_CL_DIM_NZ.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__basel_expsr_cl_dim_nz.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE EXPSR_CL_ID IN (
        SELECT EXPSR_CL_ID FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__basel_expsr_cl_dim_nz.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__basel_expsr_cl_dim_nz.branch_decide", key="csv") }}}}')
    )
    """
):
    pass

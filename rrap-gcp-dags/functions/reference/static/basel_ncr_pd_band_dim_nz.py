...
DOWNSTREAM_ASSET = f"reference.NCR_PD_BAND_DIM_NZ"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
}
INPUT_PATH = 'NCR_PD_BAND_DIM_NZ.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__ncr_pd_band_dim_nz.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE NCR_PD_BAND_ID IN (
        SELECT NCR_PD_BAND_ID FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__ncr_pd_band_dim_nz.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__ncr_pd_band_dim_nz.branch_decide", key="csv") }}}}')
    )
    """
):
    pass

...
DOWNSTREAM_ASSET = f"reference.BASEL_DFT_LKP"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
}
INPUT_PATH = 'BASEL_DFT_LKP.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__basel_dft_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(DIS_RT_CD) IN (
        SELECT TRIM(DIS_RT_CD) FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__basel_dft_lkp.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__basel_dft_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass

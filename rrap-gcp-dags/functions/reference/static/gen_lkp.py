...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__gen_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    SET 
        CRNT_F = 'N',
        EFF_TO_TMSTMP = UPPER(strftime(current_timestamp, '%d%b%y:%I:%M:%S'))
        UPDT_PROCESS_TMSTMP = current_timestamp
    WHERE TRIM(GENL_LKP_ID) IN (
        SELECT TRIM(GENL_LKP_ID) FROM '{{{{ task_instance.xcom_pull(task_ids="static__gen_lkp.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM '{{{{ task_instance.xcom_pull(task_ids="static__gen_lkp.sensor_wait_for_table", key="csv") }}}}'
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET GENL_LKP_ID = TRIM(GENL_LKP_ID)
    """
):
    pass

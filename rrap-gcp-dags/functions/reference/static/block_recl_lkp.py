...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__block_recl_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(BLOCK_RECL_CD) IN (
        SELECT TRIM(BLOCK_RECL_CD) FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__block_recl_lkp.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__block_recl_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET BLOCK_RECL_CD = TRIM(BLOCK_RECL_CD)
    """
):
    pass

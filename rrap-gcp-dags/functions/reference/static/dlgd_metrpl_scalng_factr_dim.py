...
DOWNSTREAM_ASSET = f"reference.DLGD_METRPL_SCALNG_FACTR_DIM"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
    'duckdb_load': ['duckdb_postprocess_trim'],
}
INPUT_PATH = 'DLGD_METRPL_SCALNG_FACTR_DIM.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__dlgd_metrpl_scalng_factr_dim.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE UPPER(TRIM(METRPL_AREA_NM)) IN (
        SELECT UPPER(TRIM(METRPL_AREA_NM)) FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__dlgd_metrpl_scalng_factr_dim.branch_decide", key="csv") }}}}')
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__dlgd_metrpl_scalng_factr_dim.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET METRPL_AREA_NM = TRIM(METRPL_AREA_NM)
    """
):
    pass

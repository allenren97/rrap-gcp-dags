...
DOWNSTREAM_ASSET = f"reference.BASEL_NCR_LTV_DIM"
DEPENDENCIES = {
    'branch_decide': ['duckdb_update'],
    'duckdb_update' : ['duckdb_load'],
}
INPUT_PATH = 'BASEL_NCR_LTV_DIM.csv'
...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__basel_ncr_ltv_dim.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE NCR_LTV_ID IN (
        SELECT NCR_LTV_ID FROM '{{{{ task_instance.xcom_pull(task_ids="static__basel_ncr_ltv_dim.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM '{{{{ task_instance.xcom_pull(task_ids="static__basel_ncr_ltv_dim.branch_decide", key="csv") }}}}'
    )
    """
):
    pass

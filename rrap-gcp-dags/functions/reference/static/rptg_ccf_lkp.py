...
def branch_decide():
    ...
    if os.path.exists(path):
        ...
        return ["static__rptg_ccf_lkp.duckdb_update"]
...
def duckdb_update(
    ...
    WHERE TRIM(BASEL_PRD_TP_CD) IN (
        SELECT TRIM(BASEL_PRD_TP_CD) FROM '{{{{ task_instance.xcom_pull(task_ids="static__rptg_ccf_lkp.branch_decide", key="csv") }}}}'
    ) AND CRNT_F = 'Y'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        ...
        FROM read_csv('{{{{ task_instance.xcom_pull(task_ids="static__rptg_ccf_lkp.branch_decide", key="csv") }}}}')
    )
    """
):
    pass
...
def duckdb_postprocess_trim(
    ...
    sql=f"""
    UPDATE {DOWNSTREAM_ASSET}
    SET BASEL_PRD_TP_CD = TRIM(BASEL_PRD_TP_CD)
    """
):
    pass

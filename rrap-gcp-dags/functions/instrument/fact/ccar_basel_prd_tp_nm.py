...
DEPENDENCIES = {
    'duckdb_clear': ['export_ks', 'export_spl','export_mor','export_tng'],
...
def export_ks(
    ...
    sql = f"""
        with MERG02 as  (
            ...
            left join ( select basel_acct_id, prd_cd from {UPSTREAM_ASSET[1]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')b
            on a.basel_acct_id = b.basel_acct_id

            left join ( select basel_acct_id, sub_prd_cd from {UPSTREAM_ASSET[2]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')c
            on a.basel_acct_id=c.basel_acct_id

            left join ( select basel_acct_id, heloc_f from {UPSTREAM_ASSET[3]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')d
            ...
            from {UPSTREAM_ASSET[0]} a
            left join ( select basel_acct_id, REVISED_EXPSR_OV_125K_F from {UPSTREAM_ASSET[4]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')b
            on a.basel_acct_id = b.basel_acct_id

            left join ( select basel_acct_id, BASEL_PRD_CD from {UPSTREAM_ASSET[5]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')c
            on a.basel_acct_id=c.basel_acct_id
            ...
            left join ( select * from {UPSTREAM_ASSET[8]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and stream='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="stream") }}}}')b
            on a.basel_acct_id = b.basel_acct_id

            left join ( select * from {UPSTREAM_ASSET[9]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and stream='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="stream") }}}}')c
            on a.basel_acct_id=c.basel_acct_id

            left join ( select * from {UPSTREAM_ASSET[10]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and stream='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="stream") }}}}')d
            on a.basel_acct_id=d.basel_acct_id
...
def export_spl(
    ...
    sql=f"""
        with BASEL_PSNL_LOAN_ACCT_DRVD_VARS_2 as (
            ...
            left outer join (select BASEL_ACCT_ID, PRD_ID from {UPSTREAM_ASSET[12]} where obsn_dt = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}') III 
            ...
            left join ( select basel_acct_id, PD_BASEL_SEG_NUM from {UPSTREAM_ASSET[8]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and stream='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="stream") }}}}')b
            on a.basel_acct_id = b.basel_acct_id

            left join ( select basel_acct_id, LGD_BASEL_SEG_NUM from {UPSTREAM_ASSET[9]} where obsn_dt='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' and stream='{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="stream") }}}}')c
            on a.basel_acct_id=c.basel_acct_id
...
def duckdb_load(
    ...
    FROM (
        SELECT OBSN_DT, BASEL_ACCT_ID, CCAR_BASEL_PRD_TP_NM, STREAM
          FROM 
            read_parquet('{{{{ task_instance.xcom_pull(task_ids="fact__ccar_basel_prd_tp_nm.export_ks", key="parquet") }}}}')

        UNION ALL 
        SELECT OBSN_DT, BASEL_ACCT_ID, CCAR_BASEL_PRD_TP_NM, STREAM
          FROM 
            read_parquet('{{{{ task_instance.xcom_pull(task_ids="fact__ccar_basel_prd_tp_nm.export_spl", key="parquet") }}}}')

        UNION ALL 
        SELECT OBSN_DT, BASEL_ACCT_ID, CCAR_BASEL_PRD_TP_NM, STREAM
          FROM 
            read_parquet('{{{{ task_instance.xcom_pull(task_ids="fact__ccar_basel_prd_tp_nm.export_mor", key="parquet") }}}}')

        UNION ALL 
        SELECT OBSN_DT, BASEL_ACCT_ID, CCAR_BASEL_PRD_TP_NM, STREAM
          FROM 
            read_parquet('{{{{ task_instance.xcom_pull(task_ids="fact__ccar_basel_prd_tp_nm.export_tng", key="parquet") }}}}')

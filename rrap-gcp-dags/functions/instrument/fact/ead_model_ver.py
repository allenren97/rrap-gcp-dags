...
DOWNSTREAM_ASSET = "instruments.EAD_MODEL_VER"
DEPENDENCIES = {
...
def export_result(
    duckdb_conn_id="duckdb-conn",
    config_file="ead_model_ver.export_result.sql",
...
def duckdb_load(
    ...
    FROM (
        SELECT 
            *
        FROM 
            read_parquet('{{{{ task_instance.xcom_pull(task_ids="fact__ead_model_ver.export_result", key="parquet") }}}}')
    )
    """,
):
    pass

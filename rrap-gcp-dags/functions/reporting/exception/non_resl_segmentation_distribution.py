...
from bns.reports import (
    generate_segmentation_distribution_report,
    _get_reports_dir,
    _get_models
)
REPORT_DIR = _get_reports_dir()
REPORT_NAME = 'non_resl_segmentation_distribution_report.xlsx'
STREAM = 'non_resl'
...
def generate_report(pool="duckdb_pool", pool_slots=16):
    context = get_current_context()
    obsn_dt = context['ti'].xcom_pull(task_ids="handle_month_context", key="rundate")
    generate_segmentation_distribution_report(UPSTREAM_ASSET, DOWNSTREAM_ASSET, obsn_dt, STREAM)
...
from bns.reports import (
    generate_securitization_cl_report,
    _get_reports_dir,
)

REPORT_DIR = _get_reports_dir(report_type='securitization')
SECURITIZATION_TP_CD = 'CL'
ACCOUNTS_PARQUET = 'cl_accounts_matched.parquet'
REPORT_PARQUET = 'cl_securitization_report.parquet'
...
def generate_report(pool="duckdb_pool", pool_slots=16):
    context = get_current_context()
    obsn_dt = context['ti'].xcom_pull(task_ids="handle_month_context", key="rundate")
    generate_securitization_cl_report(obsn_dt, REPORT_DIR)
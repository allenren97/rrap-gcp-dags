...
DOWNSTREAM_ASSET = "features.SUB_PRD_CD"
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                SUB_PRD_CD
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__sub_prd_cd.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__sub_prd_cd.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__sub_prd_cd.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__sub_prd_cd.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

...
DOWNSTREAM_ASSET = "features.H_C"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_h_c"],
    "export_spl": ["duckdb_delete_h_c"],
    "export_mor": ["duckdb_delete_h_c"],
    "export_tng": ["duckdb_delete_h_c"],
    "duckdb_delete_h_c": ["duckdb_load_h_c"],
}
...
def export_ks(
    ...
    SELECT
        ...
        dim.BASEL_ACCT_ID,
        'KS' AS SRC_SYS_CD,
        CASE WHEN hc.H_C IS NOT NULL THEN hc.H_C
        ELSE NULL
        END AS H_C
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                H_C
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__h_c.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__h_c.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__h_c.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__h_c.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

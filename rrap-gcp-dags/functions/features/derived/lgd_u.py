...
DOWNSTREAM_ASSET = "features.LGD_U"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_lgd_u"],
    "export_spl": ["duckdb_delete_lgd_u"],
    "export_mor": ["duckdb_delete_lgd_u"],
    "export_tng": ["duckdb_delete_lgd_u"],
    "duckdb_delete_lgd_u": ["duckdb_load_lgd_u"],
}
...
def export_ks(
    ...
    SELECT
        ...
        dim.BASEL_ACCT_ID,
        'KS' AS SRC_SYS_CD,
        CASE WHEN hc.LGD_U IS NOT NULL THEN hc.LGD_U
        ELSE NULL
        END AS LGD_U
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                LGD_U
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_u.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_u.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_u.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_u.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

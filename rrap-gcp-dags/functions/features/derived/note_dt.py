...
DOWNSTREAM_ASSET = "features.NOTE_DT"

DEPENDENCIES = {
    "export_ks": ["duckdb_delete_note_dt"],
    "export_spl": ["duckdb_delete_note_dt"],
    "export_mor": ["duckdb_delete_note_dt"],
    "export_tng": ["duckdb_delete_note_dt"],
    "duckdb_delete_note_dt": ["duckdb_load_note_dt"],
}
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                NOTE_DT
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__note_dt.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__note_dt.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__note_dt.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__note_dt.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

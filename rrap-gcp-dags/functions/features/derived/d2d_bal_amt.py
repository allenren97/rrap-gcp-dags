...
DOWNSTREAM_ASSET = "features.D2D_BAL_AMT"
DEPENDENCIES = {
    "export_d2d": ["export"],
...
def export_d2d(
    ...
    sql=r"""
        ...
        SELECT
            ...
            SUM(
                CASE
                    WHEN main.sum_srvc_code IN ('CHQ', 'SAV') THEN COALESCE(main.ACCT_BAL, 0)
                    ELSE 0
                END
            ) AS D2D_BAL_AMT
        FROM
...
def export(
    ...
    sql=r"""
        ...
        SELECT
            ...
            main.D2D_BAL_AMT::decimal(20,6) AS D2D_BAL_AMT
        FROM
            '{{ task_instance.xcom_pull(task_ids="derived__d2d_bal_amt.export_d2d", key="parquet") }}' AS main
...
def duckdb_load(
    ...
    FROM (
        select * from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__d2d_bal_amt.export", key="parquet") }}}}'])
    )
    """,
):
    pass

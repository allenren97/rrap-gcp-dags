...
DOWNSTREAM_ASSET = "features.DEFAULT_IND"
DEPENDENCIES = {
...
RENDER_SQL = """
    WITH
        ...
        batch_accounts AS MATERIALIZED (
            ...
            FROM '{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_account_buckets", key="parquet") }}' B
            CROSS JOIN params P
    ...
    SELECT
        ...
        CASE WHEN _status1 = 'CUR' AND default_date IS NOT NULL THEN 1 WHEN _status1 = 'CUR' THEN 0 END AS DEFAULT_IND,
        'MOR' AS SRC_SYS_CD
    FROM with_default
"""
...
def duckdb_load(
    ...
    FROM (
        select * from read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_1", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_2", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_3", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_4", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_5", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__default_ind.export_mor_batch_6", key="parquet") }}}}'
        ], union_by_name = true)
    )
    """,
):

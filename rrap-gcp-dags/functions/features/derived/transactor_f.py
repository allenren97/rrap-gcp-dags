...
DOWNSTREAM_ASSET = 'features.TRANSACTOR_F'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_transactor_f'],
    'export_spl': ['duckdb_clear_transactor_f'],
    'export_mor': ['duckdb_clear_transactor_f'],
    'export_tng': ['duckdb_clear_transactor_f'],
    'duckdb_clear_transactor_f': ['duckdb_derive_transactor_f']
}
...
def export_ks(
    ...
    sql=rf"""
        SELECT
            ...
            'KS' AS SRC_SYS_CD,
            CASE 
                WHEN TRIM(TRNSCTR_IND) IS NOT NULL THEN TRIM(TRNSCTR_IND)
                ELSE NULL
            END AS TRANSACTOR_F
...
def duckdb_derive_transactor_f(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        SELECT
            ...
            TRANSACTOR_F
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__transactor_f.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__transactor_f.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__transactor_f.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__transactor_f.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """,
):

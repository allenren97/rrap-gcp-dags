...
DOWNSTREAM_ASSET = 'features.MAX_DLQNT_DAY_CNT'
DEPENDENCIES = {
    'duckdb_clear_max_dlqnt_day_cnt': ['duckdb_derive_max_dlqnt_day_cnt'],
}


def duckdb_clear_max_dlqnt_day_cnt(
...
def duckdb_derive_max_dlqnt_day_cnt(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    BY NAME FROM (
		...
		SELECT
			...
			B.MTH_TM_ID,
			B.BASEL_CUST_ID,
			MAX(B.DLQNT_DAY_CNT) AS MAX_DLQNT_DAY_CNT
		FROM
			(SELECT * FROM MOR
			UNION ALL
			SELECT * FROM KS

...
DOWNSTREAM_ASSET = 'features.TOT_EXPSR_ABOVE_1500K_LMT_F'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_tot_expsr_above_1500k_lmt_f'],
    'export_spl': ['duckdb_clear_tot_expsr_above_1500k_lmt_f'],
    'export_mor': ['duckdb_clear_tot_expsr_above_1500k_lmt_f'],
    'export_tng': ['duckdb_clear_tot_expsr_above_1500k_lmt_f'],
    'duckdb_clear_tot_expsr_above_1500k_lmt_f': ['duckdb_derive_tot_expsr_above_1500k_lmt_f']
}
...
def export_ks(
    ...
    sql=rf"""
        SELECT
            ...
            'KS' AS SRC_SYS_CD,
            CASE 
                WHEN TRIM(TOT_EXPSR_ABOVE_1500K_LMT_F) IS NOT NULL 
                THEN TRIM(TOT_EXPSR_ABOVE_1500K_LMT_F)
                ELSE NULL
            END AS TOT_EXPSR_ABOVE_1500K_LMT_F
...
def duckdb_clear_tot_expsr_above_1500k_lmt_f(
    duckdb_conn_id='duckdb-conn',
    sql=rf"""
...
def duckdb_derive_tot_expsr_above_1500k_lmt_f(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        ...
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__tot_expsr_above_1500k_lmt_f.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__tot_expsr_above_1500k_lmt_f.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__tot_expsr_above_1500k_lmt_f.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__tot_expsr_above_1500k_lmt_f.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """,
):

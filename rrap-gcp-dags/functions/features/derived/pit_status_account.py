...
DOWNSTREAM_ASSET = "features.PIT_STATUS_ACCOUNT"
DEPENDENCIES = {
...
def export_ks(
    ...
    sql=f"""
        ...
        SELECT
            ...
            nullif(trim(s.step_pln_agrmnt_num), '') as STEP_PLN_AGRMNT_NUM,
            ...
            END AS STEP_DFLT_F,
            CASE WHEN UPPER(TRIM(BLOCK_RECL_CD)) IN ('SR','SF','SP','B4','SS','S','XS') THEN 'N' ELSE 'Y'
            END AS CRS_DFLT_INCL_F
        FROM
            SNAP s
    ...
    sql=r"""
        SELECT
            ...
            CASE
                ...
                AND TRIM(PD_OFF_F) = 'Y'
                AND COALESCE(CRNT_BAL_AMT, 0) <= 0
                AND (
                    greatest (CRNT_BAL_AMT, COALESCE(- TOT_SUSP_BAL_AMT, 0)) <= 0
                ) THEN 'CLO'
                WHEN TRIM(UPPER(COMM_TP)) = 'RESIDENTIAL'
                AND (
                    COALESCE(DLQNT_DAY, 0) >= 90
                    OR TRIM(FRCLSR_F) = 'Y'
                    OR TRIM(LRA_STAT) = 'Y'
                ) THEN 'DEF'
                ELSE 'CUR'
            END AS PIT_STATUS_ACCOUNT,
            CASE
                WHEN coalesce(trim(STEP_PLN_AGRMNT_NUM),'') != '' THEN 'N'
            END AS STEP_DFLT_F,
            CASE WHEN UPPER(TRIM(COMM_TP)) NOT IN ('RESIDENTIAL') THEN 'N' ELSE 'Y'
            END AS CRS_DFLT_INCL_F
        FROM
            ingestion.MORT_MTH_SNAPSHOT
        WHERE
    ...
    sql=r"""
        SELECT
            ...
            CASE
                ...
                WHEN COALESCE(TRIM(STEP_PLN_AGRMNT_NUM), '') != '' THEN 'N'
            END AS STEP_DFLT_F,
            CASE    WHEN UPPER(TRIM(TRNST_NUM)) IN ('18192', '99432') THEN 'N'
                    WHEN UPPER(TRIM(COMM_LOAN_CD)) IN ('1') THEN 'N'
                    ELSE 'Y'
            END AS CRS_DFLT_INCL_F
        FROM
            ingestion.BASEL_PSNL_LOAN_MTH_SNAPSHOT
...
def duckdb_load(
    ...
    FROM (
        select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account.export_ks", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account.export_spl", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account.export_mor", key="parquet") }}}}'], union_by_name = true)
    )
    """,
    max_active_tis_per_dag=1,
):

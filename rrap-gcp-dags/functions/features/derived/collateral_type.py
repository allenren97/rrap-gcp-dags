...
DOWNSTREAM_ASSET = "features.COLLATERAL_TYPE"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_collateral_type"],
    "export_spl": ["duckdb_delete_collateral_type"],
    "export_mor": ["duckdb_delete_collateral_type"],
    "export_tng": ["duckdb_delete_collateral_type"],
    "duckdb_delete_collateral_type": ["duckdb_load_collateral_type"],
}
...
def export_ks(
    ...
    SELECT
        ...
        dim.BASEL_ACCT_ID,
        'KS' AS SRC_SYS_CD,
        CASE WHEN TRIM(hc.COLLATERAL_TYPE) IS NOT NULL THEN TRIM(hc.COLLATERAL_TYPE)
        ELSE NULL
...
def duckdb_delete_collateral_type(
    duckdb_conn_id="duckdb-conn",
    sql=f"""
...
def duckdb_load_collateral_type(
    duckdb_conn_id="duckdb-conn",
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                collateral_type
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__collateral_type.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__collateral_type.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__collateral_type.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__collateral_type.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

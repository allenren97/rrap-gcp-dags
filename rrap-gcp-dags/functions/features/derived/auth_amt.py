...
DOWNSTREAM_ASSET = "features.AUTH_AMT"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_auth_amt"],
    "export_spl": ["duckdb_delete_auth_amt"],
    "export_mor": ["duckdb_delete_auth_amt"],
    "export_tng": ["duckdb_delete_auth_amt"],
    "duckdb_delete_auth_amt": ["duckdb_load_auth_amt"],
}
...
def export_ks(
    ...
    SELECT
        ...
        ks.BASEL_ACCT_ID,
        'KS' AS SRC_SYS_CD,
        ROUND(CR_LMT_AMT, 8) AS AUTH_AMT
...
def export_mor(
    ...
    SELECT
        ...
        mor.BASEL_ACCT_ID,
        'MOR' AS SRC_SYS_CD,
        base.AUTHORIZEDAMOUNT AS AUTH_AMT
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                AUTH_AMT
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__auth_amt.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__auth_amt.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__auth_amt.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__auth_amt.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

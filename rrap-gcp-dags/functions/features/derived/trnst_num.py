...
DOWNSTREAM_ASSET = "features.TRNST_NUM"
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                TRNST_NUM
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__trnst_num.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__trnst_num.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__trnst_num.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__trnst_num.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

...
UPSTREAM_ASSET = ["features.CUST22_V2"]
DOWNSTREAM_ASSET = "features.CUST22_SLP3M_V2"
DEPENDENCIES = {
...
def duckdb_load(
    ...
    FROM (
      with min_dt as (
        ...
        select a.*, b.CUST22_V2
        From min_dt a inner join features.CUST22_V2 b 
        on a.BASEL_CUST_ID = b.BASEL_CUST_ID and a.min_dt = b.obsn_dt
    ), curr as (
        select * from features.CUST22_V2 where obsn_dt = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
    ), base as (
        select curr.BASEL_CUST_ID, curr.CUST22_V2, prev.CUST22_V2 as cust22_v2_prev, 
        ...
        (case 
            ...
            else (cust22_v2 - cust22_v2_prev) / age 
        end) as cust22_slp3m_v2, 

...
DOWNSTREAM_ASSET = "features.PIT_STATUS_CROSS_DEFAULT"
DEPENDENCIES = {
...
def export(
    ...
    sql=r"""
        WITH
            step AS (
                SELECT
                    BASEL_ACCT_ID,
                    CASE
                        WHEN TRIM(PIT_STATUS_ACCOUNT) = 'CUR' THEN 'DEF'
                        ELSE PIT_STATUS_ACCOUNT
                    END AS PIT_STATUS_CROSS_DEFAULT,
                    CASE
                        WHEN TRIM(PIT_STATUS_ACCOUNT) = 'CUR' THEN 'Y'
                        ELSE 'N'
                ...
                WHERE
                    STEP_PLN_AGRMNT_NUM IN (
                        ...
                        WHERE
                            ...
                            AND TRIM(PIT_STATUS_ACCOUNT) IN ('CHG', 'DEF')
                            AND STEP_DFLT_F <> 'W'
                            AND CRS_DFLT_INCL_F = 'Y'
        ...
        SELECT
            ...
            main.SRC_SYS_CD,
            CASE
                WHEN step.PIT_STATUS_CROSS_DEFAULT IS NULL THEN main.PIT_STATUS_ACCOUNT
                ELSE step.PIT_STATUS_CROSS_DEFAULT
            END AS PIT_STATUS_CROSS_DEFAULT,
            CASE
            ...
            END AS CROSS_DFLT_PIT_OVERRIDE_F
        FROM
            features.PIT_STATUS_ACCOUNT AS main
...
def duckdb_load(
    ...
    FROM (
        select * from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_cross_default.export", key="parquet") }}}}'])
    )
    """,
    max_active_tis_per_dag=1,
):

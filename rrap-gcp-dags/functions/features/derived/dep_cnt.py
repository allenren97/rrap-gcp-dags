...
DOWNSTREAM_ASSET = 'features.DEP_CNT'
DEPENDENCIES = {
    'duckdb_clear_dep_cnt': ['duckdb_derive_dep_cnt'],
}


def duckdb_clear_dep_cnt(
...
def duckdb_derive_dep_cnt(
    ...
    WITH cust_txn_raw AS (
        ...
        WHERE b.TIME_KEY = {{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="mth_tm_id") }}}}
            AND b.CUST_KEY IS NOT NULL
            AND TRIM(b.ACCT_LCST) IN ('A','I','D') 
            AND TRIM(b.SUM_SRVC_CODE) IN ('CHQ', 'SAV')
    ),
    cust_txn_aggr AS (
        SELECT
            TIME_KEY,
            CUST_BASE_KEY,
            SUM(CASE 
                WHEN TXN_GRP_KEY IN (300,600,1400,3700,6200,7100,20600,18900,21700,24400,28500,29800,31000,32200,32800,33700,43200,49000)
                THEN ACTY_CNT 
                ELSE 0 
            END) AS DEP_CNT
        FROM cust_txn_raw
    ...
    cust_id_map AS (
        ...
        JOIN {UPSTREAM_ASSET[4]} c ON TRIM(x.CUST_ID) = TRIM(c.CUST_CID)
    )
    SELECT

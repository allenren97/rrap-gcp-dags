...
DOWNSTREAM_ASSET = "features.AT86_AVG6M"
DEPENDENCIES = {
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
            ...
            L15 AS (
                -- sum of MTH_SINCE_LAST_60_DAY_DLQNT_CNT per BASEL_CUST_ID over months that exist in 
                -- BASEL_CUST_ACCT_RLTNP_SNAPSHOT within a rolling 6-month window
                SELECT
                    WSS.SUM_MTH_SINCE_LAST_60_DAY_DLQNT_CNT,
                    NSS.BASEL_CUST_ID
                FROM (
                    SELECT
                        A.BASEL_CUST_ID,
            ...
            SELECT
                ...
                snap.BASEL_CUST_ID,
                ROUND(L15.SUM_MTH_SINCE_LAST_60_DAY_DLQNT_CNT / L26.CNT_BASEL_CUST_ID_26, 4) AS AT86_AVG6M
            FROM snap

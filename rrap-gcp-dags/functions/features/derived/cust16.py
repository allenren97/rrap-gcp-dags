...
DOWNSTREAM_ASSET = "features.CUST16"
DEPENDENCIES = {
...
def duckdb_load(
    ...
    FROM (
        select OBSN_DT, BASEL_CUST_ID, SUM(CUST16) AS CUST16 
        from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__cust16.export_ks", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust16.export_mor", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust16.export_spl", key="parquet") }}}}'], union_by_name = true)

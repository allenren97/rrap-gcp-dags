...
DOWNSTREAM_ASSET = "features.SECRTZTN_TP_DESC"
DEPENDENCIES = {
    'export_auto': ['duckdb_clear_secrtztn_tp_desc'],
    'export_cc': ['duckdb_clear_secrtztn_tp_desc'],
    'export_cl': ['duckdb_clear_secrtztn_tp_desc'],
    'duckdb_clear_secrtztn_tp_desc': ['duckdb_load_secrtztn_tp_desc'],
}
...
def export_auto(
    ...
    sql=f"""
        ...
        RIGHT JOIN {UPSTREAM_ASSET[3]} a ON
            CAST(TRIM(b.CRNT_BR_LOCTN_TRNST) || TRIM(LOAN_NUM) AS BIGINT) = a.ACCOUNT_NUMBER
            AND a.EFFECTIVE_DATE = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
        WHERE
...
def export_cl(
    ...
    sql=f"""
        SELECT
            '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' AS OBSN_DT,
            'CREDIT LINE' AS SECRTZTN_TP_DESC,
            ks.BASEL_ACCT_ID
...
def duckdb_clear_secrtztn_tp_desc(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
...
def duckdb_load_secrtztn_tp_desc(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    INSERT INTO {DOWNSTREAM_ASSET} (
        SELECT
            ...
            SECRTZTN_TP_DESC
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__secrtztn_tp_desc.export_auto", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__secrtztn_tp_desc.export_cc", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__secrtztn_tp_desc.export_cl", key="parquet") }}}}',
        ], union_by_name=true)
    )
    """
):

...
DOWNSTREAM_ASSET = "features.CUST21"
DEPENDENCIES = {
...
def export_ks(
    ...
    sql=f"""
        SELECT SUM(
            CASE
                WHEN TRIM(CHRG_OFF_CD) <> '1' and (TRIM(BLOCK_RECL_CD)='B5' or TRIM(CHRG_OFF_CD) in ('N','Q') or BNS_DLQNT_DAY >= 120)
                THEN b.TOT_NEW_BAL_AMT
                ELSE 0
            END
        ) AS CUST21, a.BASEL_CUST_ID,
    ...
    sql=f"""
        SELECT SUM(
            CASE
                ...
                WHEN (
                    CASE 
                    WHEN DAYNAME(LAST_DAY(DATE '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')) = 'Saturday'
                    THEN DATE_ADD(DATE '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}', -INTERVAL 1 DAY)
                    WHEN DAYNAME(LAST_DAY(DATE '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}')) = 'Sunday'
                    THEN DATE_ADD(DATE '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}', -INTERVAL 2 DAY)
                    ELSE '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
                END
                ) - WK_FRST_UNPAID_DT >= INTERVAL 90 DAY AND
                    (FRCLSR_F <> 'Y' OR FRCLSR_F IS NULL) AND NOT 
                    (
...
def export_spl(
    ...
    sql=rf"""
        SELECT SUM(
            CASE
                WHEN RECD_STAT_CD = 5 OR (RECD_STAT_CD = 4 AND DAY_ODUE >= 90)
...
def duckdb_load(
    ...
    FROM (
        select OBSN_DT, BASEL_CUST_ID, SUM(CUST21) AS CUST21 
        from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__cust21.export_ks", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust21.export_mor", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust21.export_spl", key="parquet") }}}}'], union_by_name = true)

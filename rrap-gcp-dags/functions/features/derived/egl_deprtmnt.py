...
DOWNSTREAM_ASSET = 'features.EGL_DEPRTMNT'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_egl_deprtmnt'],
    'export_spl': ['duckdb_clear_egl_deprtmnt'],
    'export_mor': ['duckdb_clear_egl_deprtmnt'],
    'export_tng': ['duckdb_clear_egl_deprtmnt'],
    'duckdb_clear_egl_deprtmnt': ['duckdb_derive_egl_deprtmnt']
}
...
def export_ks(
    ...
    SELECT
        ...
        LTRIM(RTRIM(egl.EGL_DEPRTMNT)) AS EGL_DEPRTMNT
    ...
    SELECT
        ...
        'KS' AS SRC_SYS_CD,
        CASE 
            WHEN LTRIM(RTRIM(pl1.EGL_DEPRTMNT)) IS NOT NULL 
            THEN LTRIM(RTRIM(pl1.EGL_DEPRTMNT)) 
            ELSE NULL 
        END AS EGL_DEPRTMNT
...
def export_spl(
    ...
    SELECT
        ...
        'SPL' AS SRC_SYS_CD,
        CASE 
            WHEN TRIM(egl.EGL_DEPRTMNT) IS NOT NULL 
            THEN TRIM(egl.EGL_DEPRTMNT)
            ELSE NULL 
        END AS EGL_DEPRTMNT
    ...
    LEFT JOIN {UPSTREAM_ASSET[11]} egl ON
        TRIM(prd.PRD_ID) = TRIM(egl.PRD_CD)
    WHERE 
...
def export_mor(
    ...
    SELECT
        ...
        'MOR' AS SRC_SYS_CD,
        CASE 
            WHEN TRIM(CAST(CAST(egl.EGL_DEPRTMNT AS INT) AS VARCHAR)) IS NOT NULL
            THEN TRIM(CAST(CAST(egl.EGL_DEPRTMNT AS INT) AS VARCHAR)) 
            ELSE NULL
        END AS EGL_DEPRTMNT
...
def export_tng(
    ...
    GROUP BY
        dim.BASEL_ACCT_ID,
        TRIM(egl.EGL_DEPRTMNT)
    """
):
    pass

def duckdb_clear_egl_deprtmnt(
    duckdb_conn_id='duckdb-conn',
    sql=rf"""
...
def duckdb_derive_egl_deprtmnt(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        SELECT
            ...
            EGL_DEPRTMNT
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__egl_deprtmnt.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__egl_deprtmnt.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__egl_deprtmnt.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__egl_deprtmnt.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """
):

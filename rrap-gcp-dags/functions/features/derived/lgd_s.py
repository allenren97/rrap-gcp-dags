...
DOWNSTREAM_ASSET = "features.LGD_S"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_lgd_s"],
    "export_spl": ["duckdb_delete_lgd_s"],
    "export_mor": ["duckdb_delete_lgd_s"],
    "export_tng": ["duckdb_delete_lgd_s"],
    "duckdb_delete_lgd_s": ["duckdb_load_lgd_s"],
}
...
def export_ks(
    ...
    SELECT
        ...
        dim.BASEL_ACCT_ID,
        'KS' AS SRC_SYS_CD,
        CASE WHEN hc.LGD_S IS NOT NULL THEN hc.LGD_S
        ELSE NULL
        END AS LGD_S
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                LGD_S
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_s.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_s.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_s.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__lgd_s.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

...
DOWNSTREAM_ASSET = "features.CUST22"
DEPENDENCIES = {
...
def duckdb_load(
    ...
    FROM (
        with derived_inputs as (
            select BASEL_CUST_ID, CUST22
            from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__cust22.export_ks", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust22.export_mor", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__cust22.export_spl", key="parquet") }}}}'], union_by_name = true)
        )
        select
            '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' AS OBSN_DT,
            scrd_list.BASEL_CUST_ID,
            SUM(derived_inputs.CUST22) AS CUST22
        from '{{{{ task_instance.xcom_pull(task_ids="derived__cust22.export_cust_list", key="parquet") }}}}' scrd_list

...
DOWNSTREAM_ASSET = 'features.BASEL_PRD_ABR'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_basel_prd_abr'],
    'export_spl': ['duckdb_clear_basel_prd_abr'],
    'export_mor': ['duckdb_clear_basel_prd_abr'],
    'export_tng': ['duckdb_clear_basel_prd_abr'],
    'duckdb_clear_basel_prd_abr': ['duckdb_derive_basel_prd_abr']
}
...
def export_ks(
    ...
    SELECT
        ...
        ks.BASEL_ACCT_ID,
        sys_cd.SRC_SYS_CD,
        CASE WHEN TRIM(rptg.BASEL_PRD_ABR) IS NOT NULL
        THEN TRIM(rptg.BASEL_PRD_ABR)
        ELSE NULL
        END AS BASEL_PRD_ABR
...
def duckdb_clear_basel_prd_abr(
    duckdb_conn_id='duckdb-conn',
    sql=rf"""
...
def duckdb_derive_basel_prd_abr(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        SELECT
            ...
            BASEL_PRD_ABR
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__basel_prd_abr.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__basel_prd_abr.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__basel_prd_abr.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__basel_prd_abr.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """
):

...
DOWNSTREAM_ASSET = 'features.TOT_DEP_AMT'
DEPENDENCIES = {
    'duckdb_clear_tot_dep_amt': ['duckdb_derive_tot_dep_amt'],
}


def duckdb_clear_tot_dep_amt(
...
def duckdb_derive_tot_dep_amt(
    ...
    sql=f"""
        ...
        WITH cust_txn_raw AS (
            ...
            JOIN {UPSTREAM_ASSET[1]} d
            ON b.CUST_BASE_KEY = d.CUST_BASE_KEY
        ...
        cust_txn_mapped AS (
            SELECT
                ...
                m.TXN_GRP_KEY
            FROM cust_txn_raw a
            LEFT JOIN {UPSTREAM_ASSET[2]} m
                ON a.DLVY_KEY = m.DLVY_KEY
        ),
        cust_txn_aggr AS (
            SELECT
                TIME_KEY,
                CUST_BASE_KEY,
                SUM(CASE WHEN TXN_GRP_KEY = 'D' THEN ACTY_TOT_AMT ELSE 0 END) AS TOT_DEP_AMT
            FROM cust_txn_mapped
        ...
        cust_id_map AS (
            ...
            JOIN {UPSTREAM_ASSET[4]} c
                ON x.CUST_ID = TRIM(c.CUST_CID)
        )
        SELECT

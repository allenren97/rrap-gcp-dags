...
# (mirrors the SAS inner join to the defaulter table). KS is batched 6-way by
# MOD(HASH(BASEL_ACCT_ID), 6); SPL is a single pass.
UPSTREAM_ASSET = [
...
# SPL: status-only default definition. Single pass.
def export_spl(
    duckdb_conn_id="duckdb-conn",
    resource_tier="HIGH",
    pool_slots=96,
...
# KS: status + balance/charge gate, batched by MOD(HASH,REPLACE_COUNT)=REPLACE_ID.
RENDER_KS = """
    WITH batch_accounts AS MATERIALIZED (
        SELECT B.BASEL_ACCT_ID
        FROM '{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_account_buckets", key="parquet") }}' B
...
def duckdb_load(
    ...
    FROM (
        select * from read_parquet([
            ...
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_1", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_2", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_3", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_4", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_5", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__model_dft_f.export_ks_batch_6", key="parquet") }}}}'],
        union_by_name = true)
    )
    """,
):

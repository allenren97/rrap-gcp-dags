...
DOWNSTREAM_ASSET = "features.PIT_STATUS_ACCOUNT_GCP"
DEPENDENCIES = {
    ...
    sql=r"""
        SELECT
            ...
            CASE
                ...
                when CRNT_BAL_AMT >= 0 and (INTR_ADJ_DT is null or '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate")[:8] + '01' }}' <= INTR_ADJ_DT) then 'CUR'
                when TRIM(UPPER(FRCLSR_F)) = 'Y' and CRNT_BAL_AMT > 0 then 'DEF'
                when (INTR_ADJ_DT is not null or '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate")[:8] + '01' }}' > INTR_ADJ_DT) and
                (
                    trim(PD_OFF_F) <> 'Y' or
                    trim(PD_OFF_F) is null
                ) and
                CRNT_BAL_AMT <= 0 then 'CLO'
                when (INTR_ADJ_DT is not null or '{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate")[:8] + '01' }}' > INTR_ADJ_DT) and
                trim(PD_OFF_F) = 'Y' then 'CLO'
            END AS PIT_STATUS_ACCOUNT_GCP,
            CASE
...
def export_spl(
    ...
    sql=r"""
        ...
        WHERE
            ...
            trim(RECD_STAT_CD) IN (4, 5, 6, 7, 8, 0, 9)
    """,
):
    pass
...
def duckdb_load(
    ...
    FROM (
        select *, '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account_gcp.export_ks", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account_gcp.export_spl", key="parquet") }}}}','{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_account_gcp.export_mor", key="parquet") }}}}'], union_by_name = true)
    )
    """,
    max_active_tis_per_dag=1,
):

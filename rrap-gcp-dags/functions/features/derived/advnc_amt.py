...
DOWNSTREAM_ASSET = "features.ADVNC_AMT"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_advnc_amt"],
    "export_spl": ["duckdb_delete_advnc_amt"],
    "export_mor": ["duckdb_delete_advnc_amt"],
    "export_tng": ["duckdb_delete_advnc_amt"],
    "duckdb_delete_advnc_amt": ["duckdb_load_advnc_amt"],
}
...
def export_mor(
    ...
    SELECT
        ...
        mor.BASEL_ACCT_ID,
        'MOR' AS SRC_SYS_CD,
        base.ADVANCEDAMOUNT AS ADVNC_AMT
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                ADVNC_AMT
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__advnc_amt.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__advnc_amt.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__advnc_amt.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__advnc_amt.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

...
UPSTREAM_ASSET = [
    "features.PIT_STATUS_ACCOUNT_ORIG",
]
DOWNSTREAM_ASSET = "features.PIT_STATUS_CROSS_DEFAULT_ORIG"
DEPENDENCIES = {
...
def export(
    ...
    sql=r"""
        WITH
            step AS (
                SELECT
                    BASEL_ACCT_ID,
                    CASE
                        WHEN TRIM(PIT_STATUS_ACCOUNT_ORIG) = 'CUR' THEN 'DEF'
                        ELSE PIT_STATUS_ACCOUNT_ORIG
                    END AS PIT_STATUS_CROSS_DEFAULT_ORIG,
                    CASE
                        WHEN TRIM(PIT_STATUS_ACCOUNT_ORIG) = 'CUR' THEN 'Y'
                        ELSE 'N'
                ...
                WHERE
                    STEP_PLN_AGRMNT_NUM IN (
                        ...
                        WHERE
                            ...
                            AND TRIM(PIT_STATUS_ACCOUNT_ORIG) IN ('CHG', 'DEF')
                            AND STEP_DFLT_F <> 'W'
        ...
        SELECT
            ...
            main.SRC_SYS_CD,
            CASE
                WHEN step.PIT_STATUS_CROSS_DEFAULT_ORIG IS NULL THEN main.PIT_STATUS_ACCOUNT_ORIG
                ELSE step.PIT_STATUS_CROSS_DEFAULT_ORIG
            END AS PIT_STATUS_CROSS_DEFAULT_ORIG,
            CASE
            ...
            END AS CROSS_DFLT_PIT_OVERRIDE_F
        FROM
            features.PIT_STATUS_ACCOUNT_ORIG AS main
...
def duckdb_load(
    ...
    FROM (
        select * from read_parquet(['{{{{ task_instance.xcom_pull(task_ids="derived__pit_status_cross_default_orig.export", key="parquet") }}}}'])
    )
    """,
    max_active_tis_per_dag=1,
):

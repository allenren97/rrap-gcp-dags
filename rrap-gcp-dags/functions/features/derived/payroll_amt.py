...
DOWNSTREAM_ASSET = 'features.PAYROLL_AMT'

DEPENDENCIES = {
    'duckdb_clear_payroll_amt': ['duckdb_derive_payroll_amt'],
}


def duckdb_clear_payroll_amt(
...
def duckdb_derive_payroll_amt(
    ...
    BY NAME FROM (
        select
            basel_cust_id,
            round(sum(case when txn_grp_key_rm = 'P' then acty_tot_amt else 0 end), 2) as payroll_amt,
            '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}' as OBSN_DT,
        from (
            select distinct d.basel_cust_id, a.TXN_GRP_KEY_RM, b.ACCT_KEY, b.ACTY_TOT_AMT
            ...
            where b.cust_base_key is not null

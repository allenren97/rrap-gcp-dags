...
DOWNSTREAM_ASSET = "features.ACCT_ODT"
DEPENDENCIES = {
    "export_ks": ["duckdb_delete_acct_odt"],
    "export_spl": ["duckdb_delete_acct_odt"],
    "export_mor": ["duckdb_delete_acct_odt"],
    "export_tng": ["duckdb_delete_acct_odt"],
    "duckdb_delete_acct_odt": ["duckdb_load_acct_odt"],
}
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                ACCT_ODT
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__acct_odt.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__acct_odt.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__acct_odt.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__acct_odt.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

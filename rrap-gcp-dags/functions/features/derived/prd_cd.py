...
DOWNSTREAM_ASSET = 'features.PRD_CD'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_prd_cd'],
    'export_spl': ['duckdb_clear_prd_cd'],
    'export_mor': ['duckdb_clear_prd_cd'],
    'export_tng': ['duckdb_clear_prd_cd'],
    'duckdb_clear_prd_cd': ['duckdb_derive_prd_cd']
}
...
def duckdb_derive_prd_cd(
    ...
    FROM (
        SELECT
            ...
            PRD_CD
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__prd_cd.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__prd_cd.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__prd_cd.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__prd_cd.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """
):

...
DOWNSTREAM_ASSET = 'features.SRC_PRD_DESC'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_src_prd_desc'],
    'export_spl': ['duckdb_clear_src_prd_desc'],
    'export_mor': ['duckdb_clear_src_prd_desc'],
    'export_tng': ['duckdb_clear_src_prd_desc'],
    'duckdb_clear_src_prd_desc': ['duckdb_derive_src_prd_desc']
}
...
def export_ks(
    ...
    SELECT
        ...
        'KS' AS SRC_SYS_CD,
        CASE 
            WHEN TRIM(pl1.BASEL_SUB_PRD_NM) IS NOT NULL 
            THEN TRIM(pl1.BASEL_SUB_PRD_NM) 
            ELSE '' 
        END AS SRC_PRD_DESC
...
def export_spl(
    ...
    SELECT
        ...
        'SPL' AS SRC_SYS_CD,
        CASE 
            WHEN lkp.BASEL_SUB_PRD_NM IS NOT NULL 
            THEN lkp.BASEL_SUB_PRD_NM 
            ELSE '' 
        END AS SRC_PRD_DESC
...
def export_mor(
    ...
    SELECT
        ...
        'MOR' AS SRC_SYS_CD,
        CASE 
            WHEN TRIM(rptg.BASEL_SUB_PRD_NM) IS NOT NULL
            THEN UPPER(TRIM(rptg.BASEL_SUB_PRD_NM)) 
            ELSE ''
        END AS SRC_PRD_DESC
...
def duckdb_derive_src_prd_desc(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        SELECT
            ...
            SRC_PRD_DESC
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__src_prd_desc.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__src_prd_desc.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__src_prd_desc.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__src_prd_desc.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """
):

...
DOWNSTREAM_ASSET = 'features.LTV_BUCKET'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_ltv_bucket'],
    'export_spl': ['duckdb_clear_ltv_bucket'],
    'export_mor': ['duckdb_clear_ltv_bucket'],
    'export_tng': ['duckdb_clear_ltv_bucket'],
    'duckdb_clear_ltv_bucket': ['duckdb_derive_ltv_bucket']
}
...
def export_ks(
    ...
    sql=rf"""
        SELECT
            ...
            'KS' AS SRC_SYS_CD,
            CASE 
                WHEN TRIM(LTV_BCKT_CD) IS NOT NULL THEN TRIM(LTV_BCKT_CD)
                ELSE NULL
            END AS LTV_BUCKET
...
def duckdb_derive_ltv_bucket(
    ...
    FROM (
        SELECT
            ...
            LTV_BUCKET
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__ltv_bucket.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__ltv_bucket.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__ltv_bucket.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__ltv_bucket.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """,
):

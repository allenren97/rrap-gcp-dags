...
DOWNSTREAM_ASSET = 'features.UTIL_KSA_AVG3M'
DEPENDENCIES = {
    'duckdb_clear_util_ksaavg3m': ['duckdb_derive_util_ksaavg3m'],
}


def duckdb_clear_util_ksaavg3m(
...
def duckdb_derive_util_ksaavg3m(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    WITH base AS (
        select
        BASEL_ACCT_ID,
        TRUNC(AVG(UTIL_KSA),4) as UTIL_KSA_AVG3M

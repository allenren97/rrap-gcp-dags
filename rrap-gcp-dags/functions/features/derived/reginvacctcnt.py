...
DOWNSTREAM_ASSET = 'features.REGINVACCTCNT'

DEPENDENCIES = {
    'duckdb_clear_reginvacctcnt': ['duckdb_derive_reginvacctcnt'],
}


def duckdb_clear_reginvacctcnt(
...
def duckdb_derive_reginvacctcnt(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    INSERT INTO { DOWNSTREAM_ASSET }
        ...
        , cust_products_agg AS (
            SELECT
                t1.time_key,
				t1.cust_base_key,
                SUM(CASE WHEN t1.sum_srvc_code in ('RDS','REF','REI','RIF','RSP','TFS') THEN 1 ELSE 0 END) AS reginv
	...
	SELECT
            ...
            CAST(A.RGSTRD_INVSTMNT_BAL_ACCT_CNT AS INTEGER)  AS REGINVACCTCNT
        FROM 

...
DOWNSTREAM_ASSET = "features.LOAN_NUM"

DEPENDENCIES = {
    "export_ks": ["duckdb_delete_loan_num"],
    "export_spl": ["duckdb_delete_loan_num"],
    "export_mor": ["duckdb_delete_loan_num"],
    "export_tng": ["duckdb_delete_loan_num"],
    "duckdb_delete_loan_num": ["duckdb_load_loan_num"],
}
    ...
    sql=f"""
        ...
        FROM (
            SELECT
                ...
                LOAN_NUM
            FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__loan_num.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__loan_num.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__loan_num.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__loan_num.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
        )
    """,
):

...
DOWNSTREAM_ASSET = 'features.RNTL_PRPTY_F_IFRS9'
DEPENDENCIES = {
    'export_ks': ['duckdb_clear_rntl_prpty_f_ifrs9'],
    'export_spl': ['duckdb_clear_rntl_prpty_f_ifrs9'],
    'export_mor': ['duckdb_clear_rntl_prpty_f_ifrs9'],
    'export_tng': ['duckdb_clear_rntl_prpty_f_ifrs9'],
    'duckdb_clear_rntl_prpty_f_ifrs9': ['duckdb_derive_rntl_prpty_f_ifrs9']
}
...
def duckdb_clear_rntl_prpty_f_ifrs9(
    duckdb_conn_id='duckdb-conn',
    sql=rf"""
...
def duckdb_derive_rntl_prpty_f_ifrs9(
    duckdb_conn_id='duckdb-conn',
    sql=f"""
    ...
    FROM (
        SELECT
            ...
            RNTL_PRPTY_F_IFRS9
        FROM read_parquet([
            '{{{{ task_instance.xcom_pull(task_ids="derived__rntl_prpty_f_ifrs9.export_ks", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__rntl_prpty_f_ifrs9.export_spl", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__rntl_prpty_f_ifrs9.export_mor", key="parquet") }}}}',
            '{{{{ task_instance.xcom_pull(task_ids="derived__rntl_prpty_f_ifrs9.export_tng", key="parquet") }}}}'
        ], union_by_name=true)
    )
    """,
):

...
DOWNSTREAM_ASSET = "ingestion.BASEL_REVLVNG_CR_MTH_SNAPSHOT"
DEPENDENCIES = {
...
def sensor_wait_for_table(
    ...
    path = os.path.join(
        "/bns/rrap/data", f"{rundate}", "jb0072_fixed2_BASEL_REVLVNG_CR_MTH_SNAPSHOT/"
    )
...
def duckdb_load(
    ...
    sql=f"""
        ...
        FROM (
        SELECT -1 as REVLVNG_CR_BASEL_MTH_SNAPSHOT_ID, * exclude (basel_cust_id, crnt_bill_cd_1) FROM '{{{{ task_instance.xcom_pull(task_ids="cleaned__basel_revlvng_cr_mth_snapshot.sensor_wait_for_table", key="parquet") }}}}*.parquet'
        )
    """,
):
    pass

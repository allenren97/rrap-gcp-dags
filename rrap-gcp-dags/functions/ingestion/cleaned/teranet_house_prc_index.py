...
DOWNSTREAM_ASSET = "ingestion.TERANET_HOUSE_PRC_INDEX"
DEPENDENCIES = {
...
def sensor_wait_for_table(poke_interval=300, timeout=(60 * 60 * 24 * 8), mode='reschedule'):
    ...
    path = os.path.join("/bns/rrap/data", f"{ rundate }", 'CMA11_DATA.parquet')
...
def duckdb_load_into_ducklake(
    ...
    FROM (
    SELECT * 
    FROM '{{{{ task_instance.xcom_pull(task_ids="cleaned__teranet_house_prc_index.sensor_wait_for_table", key="parquet") }}}}'
    )
    """
):
    pass

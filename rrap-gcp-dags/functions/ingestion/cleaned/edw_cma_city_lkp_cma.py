...
DOWNSTREAM_ASSET = "reference.EDW_CMA_CITY_LKP_CMA"
DEPENDENCIES = {
    'sensor_wait_for_csv': ['reformat_file'],
    'reformat_file': ['branch_check_for_edw_cma_updates'],
    'branch_check_for_edw_cma_updates': ['duckdb_clear_edw_cma_city_lkp_cma'],
    'duckdb_clear_edw_cma_city_lkp_cma': ['duckdb_load_into_ducklake'],
    'duckdb_load_into_ducklake': ['empty_task'],
}

RAW_CSV_PATH = os.path.join("/bns/rrap/data/sas_inputs", 'rrm_edwext_CMA32_City_lookup_f_adhoc_CMA32.csv')
REFORMAT_CSV_PATH = os.path.join("/bns/rrap/data/sas_inputs", 'EDW_CMA_CITY_LKP_CMA.csv')
...
def branch_check_for_edw_cma_updates():
    # See if the static file md5sum output matches the md5sum
    # If not, update the md5sum file and proceed to next steps
    context = get_current_context()
    csv_filename = context['ti'].xcom_pull(task_ids=f"cleaned__edw_cma_lkp_cma.sensor_wait_for_csv", key="csv")
    md5sum_filename = os.path.join("/bns/rrap/data/sas_inputs", "EDW_CMA_CITY_LKP_CMA.csv.md5sum")
    ...
    if not os.path.exists(md5sum_filename):
        ...
        if md5sum_hash == existing_hash:
            ...
            return 'duckdb_clear_edw_cma_city_lkp_cma'


def duckdb_clear_edw_cma_city_lkp_cma(
    duckdb_conn_id="duckdb-conn",
    sql=rf"""TRUNCATE { DOWNSTREAM_ASSET }"""
):
    pass
...
def duckdb_load_into_ducklake(
    ...
    FROM (
    SELECT * FROM '{{{{ task_instance.xcom_pull(task_ids="cleaned__edw_cma_lkp_cma.sensor_wait_for_csv", key="csv") }}}}'
    )
    """
):
    pass

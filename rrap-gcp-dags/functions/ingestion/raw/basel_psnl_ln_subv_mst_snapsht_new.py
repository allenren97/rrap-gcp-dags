...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.BASEL_PSNL_LN_SUBV_MST_SNAPSHT_NEW"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0162_BASEL_PSNL_LN_SUBV_MST_SNAPSHT_NEW.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__basel_psnl_ln_subv_mst_snapsht_new.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

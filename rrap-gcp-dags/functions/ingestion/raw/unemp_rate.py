...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.UNEMP_RATE"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0371_UNEMP_RATE.parquet'
...
def sensor_wait_for_table(poke_interval=300, timeout=(60 * 60 * 24 * 8), mode='reschedule'):
    ...
    group_id = str(context['ti'].task_id).split('.')[0]
    print(group_id)
...
def duckdb_delete(
    ...
    WHERE TIME_KEY = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__unemp_rate.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

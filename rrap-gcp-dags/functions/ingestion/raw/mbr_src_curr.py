...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.MBR_SRC_CURR"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0831_AIRB_MBR_SRC.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__mbr_src_curr.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

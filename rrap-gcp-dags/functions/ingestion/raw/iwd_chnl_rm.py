...
DUCKLAKE_SCHEMA = "reference"
UPSTREAM_ASSET = [ "ingestion.TM_DIM" ]
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.IWD_CHNL_RM"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'IWD_CHNL_RM.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__iwd_chnl_rm.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

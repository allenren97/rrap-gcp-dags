...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.BASEL_ACCT_PFRM_FACT"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'UPDATE_KS_BSL_ACCT_ID.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__basel_acct_pfrm_fact.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.BASEL_IFRS9_ECL_PROFILE_FACT"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb00512_basel_ifrs9_ecl_profile_fact.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__basel_ifrs9_ecl_profile_fact.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

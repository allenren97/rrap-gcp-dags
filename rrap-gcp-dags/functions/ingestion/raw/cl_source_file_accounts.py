...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.CL_SOURCE_FILE_ACCOUNTS"
DEPENDENCIES = {
...
def sensor_wait_for_table(poke_interval=300, timeout=(60 * 60 * 24 * 8), mode='reschedule'):
    ...
    path = os.path.join("/bns/rrap/data/securitization/cc/incoming", f"Halifax_securitization_acct_mthly_{year_month}.csv")
...
def duckdb_load(
    ...
    FROM (
        SELECT *
        FROM read_csv(
            '{{{{ task_instance.xcom_pull(task_ids="raw__cl_source_file_accounts.sensor_wait_for_table", key="parquet") }}}}',
            delim = ',',
            header = true,
            dateformat = '%Y%m%d',
            columns = {{

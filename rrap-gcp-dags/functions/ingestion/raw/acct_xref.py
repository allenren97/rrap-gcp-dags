...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.ACCT_XREF"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'ACCT_XREF.parquet'
...
def duckdb_delete(
    ...
    WHERE POPN_DT = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__acct_xref.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

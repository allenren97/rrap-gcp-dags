...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.ASSET_SRC_CURR"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0351_airb_asst_src.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__asset_src_curr.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

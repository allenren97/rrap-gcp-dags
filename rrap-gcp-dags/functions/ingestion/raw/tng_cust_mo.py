...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.TNG_CUST_MO"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0111_TNG_CPD2_CUSTOMER_PORTFOLIO_SUMMARY_1.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__tng_cust_mo.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

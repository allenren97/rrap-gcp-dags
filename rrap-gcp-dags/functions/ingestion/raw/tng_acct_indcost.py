...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.TNG_ACCT_INDCOST"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0201_combine_costs_with_dlqnt.parquet'
...
def duckdb_delete(
    ...
    WHERE YEAR = year('{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'::date)::decimal(4,0)
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__tng_acct_indcost.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

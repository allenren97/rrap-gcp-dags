...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.CUST_XREF"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'CUST_XREF.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__cust_xref.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

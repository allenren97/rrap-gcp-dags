...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.IWD_PD_PLN"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'IWD_PD_PLN.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__iwd_pd_pln.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

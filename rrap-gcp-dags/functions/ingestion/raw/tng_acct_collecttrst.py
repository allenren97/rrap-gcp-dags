...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.TNG_ACCT_COLLECTTRST"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0332_TNG_ACCT_COLLECTTRST.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__tng_acct_collecttrst.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

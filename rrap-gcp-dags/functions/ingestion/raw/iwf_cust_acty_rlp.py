...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.IWF_CUST_ACTY_RLP"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'IWF_CUST_ACTY_RLP.parquet'
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__iwf_cust_acty_rlp.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

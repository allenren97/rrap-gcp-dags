...
DOWNSTREAM_ASSET = f"{DUCKLAKE_SCHEMA}.BASELAYER_MOR"
DEPENDENCIES = {
    'sensor_wait_for_table': ['duckdb_delete'],
    'duckdb_delete' : ['duckdb_load'],
}
INPUT_PATH = 'jb0232_BASELAYER_MOR.parquet'
...
def duckdb_delete(
    ...
    WHERE MTH_END_DT = '{{{{ task_instance.xcom_pull(task_ids="handle_month_context", key="rundate") }}}}'
    """
):
    pass
...
def duckdb_load(
    ...
    FROM (
        SELECT * FROM read_parquet('{{{{ task_instance.xcom_pull(task_ids="raw__baselayer_mor.sensor_wait_for_table", key="parquet") }}}}')
    )
    """
):
    pass

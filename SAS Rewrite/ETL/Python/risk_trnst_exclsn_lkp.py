...
# insert into table risk_trnst_exclsn_lkp table by adhoc

SQL1 = """
set hive.exec.dynamic.partition.mode=nonstrict;
insert overwrite table """ + cf.CBSDBName + """.risk_trnst_exclsn_lkp partition (eff_dt)
select 
  excluded_trnst_num
  ,current_timestamp() as insrt_process_tmstmp
  ,'""" + os.path.realpath(__file__) + """' as op_field
  ,businesseffectivedate
from """ + cf.TSZRMADBName + """.cbs_trnst_exclsn_lkp
where businesseffectivedate in (select max(businesseffectivedate) from """ + cf.TSZRMADBName + """.cbs_trnst_exclsn_lkp)
"""

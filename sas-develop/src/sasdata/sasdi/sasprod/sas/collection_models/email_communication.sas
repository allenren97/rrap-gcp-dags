...
%global internal_list internal_cc_list important_list;
...
data collection_model_email_list;
  ...
  length model $10 to_list $250 cc_list $100 jucc_list $100;
...
select to_list, cc_list into :internal_list,:internal_cc_list from collection_model_email_list where trim(model)='INTERNAL';
select to_list into :important_list from collection_model_email_list where trim(model)='IMPORTANT';
quit;
%mend;

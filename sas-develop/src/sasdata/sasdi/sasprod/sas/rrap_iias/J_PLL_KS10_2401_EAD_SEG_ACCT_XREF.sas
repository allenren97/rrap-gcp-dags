...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2401_EAD_SEG_ACCT_XREF
*  Target Database: EDRTLRPLL
*  Target Table:  EAD_SEG_ACCT_XREF
*  
...
%let jobID = %quote(A57SWEI7.BH0009HY);
%let etls_jobName = %nrquote(J_PLL_KS10_2401_EAD_SEG_ACCT_XREF);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YSC * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSC);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YSD * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSD);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YSE * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NGB * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSE);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YSF * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NGB * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGC * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSF);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F50ZH; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F50ZR(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F5101; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_EAD_SEG_ACCT_XREF         A57SWEI7.BK000YSG * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGC * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YSG);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         %etl_job_start(TARGET_TABLE=NETCON.EAD_SEG_ACCT_XREF);
...
%MACRO load_to_EAD();
      ...
      EXECUTE(DELETE FROM &net_db..EAD_SEG_ACCT_XREF WHERE MTH_TM_ID=&TM_DIM_ID) BY NZCON1;
      QUIT;
      ...
      %DO i=1 %TO &total_query;
         ...
         CONNECT USING NETCON AS IIASCON;       
            EXECUTE(
               insert into EDRTLRPLL.EAD_SEG_ACCT_XREF ( 
                     BASEL_ACCT_ID
                     , MTH_TM_ID
                     , BASEL_SEG_ID
                  , BASEL_MODEL_REL_ID
...
%load_to_EAD();
         ...
         /**  Step end RRAP_DM_INS_EAD_SEG_ACCT_XREF **/
         
      %mend etls_jobW2F5101; 
   ...
   %mend etls_processToLoopW2F50ZR;
   ...
   %let etls_parameterTable = work.W2F50ZS; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F50ZR(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YSH * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSH);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

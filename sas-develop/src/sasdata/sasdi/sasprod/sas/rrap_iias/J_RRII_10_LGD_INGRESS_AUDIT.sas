...
data count_queries(keep=c_query u_query);
	...
	where REPLICATION_METHOD='Ingress' and REPLICATION_SEQUENCE = 1;
...
execute(
	...
	WHERE REPLICATION_METHOD='Ingress' AND REPLICATION_SEQUENCE = 1) lkp
...
%macro sendemail;
		FILENAME OUTMAIL EMAIL
			SUBJECT= "Trigger Ingress to GCP Part1 in Qlik Tool";
		DATA _NULL_;
			FILE OUTMAIL
				TO= (&email_list);
			PUT "Hi,";
			PUT " ";
			PUT "Trigger Ingress/Replicate to GCP Part1 in Qlik Tool";
			PUT "Following jobs can be triggered in parallel";
			PUT "1: IIAS_YEAR_MTH_ME_SEQ_1_APPENDONLY_EDRTLRP1D";
			PUT "2: IIAS_YEAR_MTH_ME_SEQ_1_APPENDONLY_FRG_USER_DATA";
			PUT "3: IIAS_YEAR_MTH_ME_SEQ_1_FULL_RELOAD_EDRTLRP1D";
			RUN;
%mend sendemail;

%sendemail;
%PUT 'Ingress Audit Job completed, mail triggered for Ingress Part 1 replication in Qlik';

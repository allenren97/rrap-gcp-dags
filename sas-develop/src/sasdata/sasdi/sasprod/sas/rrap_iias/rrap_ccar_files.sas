...
/*20DEC2021,Zoran added for RRMSS-915 PD Curve Fix----START---*/
/*   Dataset DRAPT.TMP_PD_CURVE contains CCAR_BASEL_PRODUCT_TYPEs that exist in DR-AIRB-EXPOSURES-YYYYMMDD.csv */
/*   file but not in DR-AIRB-PD-CURVE-YYYYMMDD.csv. They are to be added from prior month */

/* The previous month records for the missing products should be fetched from EDRRAPT.BASEL_CCAR_PD_CURVE_EXTR  and */
/* added to the PD_CURVE csv file for this month   */
proc sql;
  create table DRAPT.TMP_PD_CURVE as
  select CCAR_BASEL_PRD_TP_NM as 'Basel Product Type'n, 
         PD_BAND               as 'PD Band'n   format $4., 
         put(PD_VAL*100,12.6)||"%" as 'Pd Value'n  format $13.,
         put(PD_MIN_VAL*100,12.4)  as 'Pd Min'n    format $12., 
         put(PD_MAX_VAL*100,12.4)  as 'PD Max'n    format $12. 
  from EDRRAPT.BASEL_CCAR_PD_CURVE_EXTR  
/*  from EDRRAP.BASEL_CCAR_PD_CURVE_EXTR     */
  where MTH_END_DT ="&PREV_MTH_END_DT"d 
  and   CCAR_BASEL_PRD_TP_NM IN 
  (SELECT  distinct EX.'Basel Product Type'n 
  FROM DRAPT.expsr_&YYYYMMDD   EX
EXCEPT
  SELECT distinct PD.'Basel Product Type'n 
  FROM DRAPT.dr_airb_pd_curve_&YYYYMMDD  PD  )
    order by input('Basel Product Type'n,8.);    
quit;   

/*20DEC2021,Zoran Jovicic,RRMSS-915 PD Curve Fix. */
/* Add CCAR_BASEL_PRODUCT_TYPE that is missing in current  month's PD_CURVE file from    */
/* prior month's EXPOSURES file. For details on business logic contact Faranak Pourkafi. */ 
proc append data= DRAPT.TMP_PD_CURVE OUT=&lib..DR_AIRB_PD_CURVE_&YYYYMMDD. force;
run;

/*20DEC2021,Zoran added for RRMSS-915 PD Curve Fix----END---*/

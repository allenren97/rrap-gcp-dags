...
%let etls_jobName = %nrquote(J_pll_exception_report_2_KS_Scorecard_Distribution);
...
%let file_name_ks=KS_PLL_Scorecard_Distribution_&file_name_date..xls;
...
line "KS Parallel Run Scorecard Distribution &file_name_date - SI above threshold";
endcomp;
run;
...
compute after MODEL_NM;
 ...
 endcomp;
compute before _PAGE_;
line "KS Parallel Run Scorecard Distribution &file_name_date";
endcomp;
run;
...
SUBJECT= "[RRAP] SI above threshold for KS Parallel Run Scorecard Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&si_email_list);
     put;
...
%else %do;
FILENAME OUTMAIL EMAIL ATTACH=("&file_path.&file_name_ks")
SUBJECT= "[RRAP] KS Parallel Run Scorecard Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&email_list);

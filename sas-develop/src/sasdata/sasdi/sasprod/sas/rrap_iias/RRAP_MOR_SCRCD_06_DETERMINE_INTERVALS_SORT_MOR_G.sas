...
%if %sysfunc(exist(&target_lib..SCORECARD_VARS_MOR_09_3)) %then %do;
  proc sql;  
    connect USING NZUSER as nzcon ;
    execute( insert into &target_lib3..SCORECARD_VARS_MOR_09_3
             select *
             from &target_lib3..SCORECARD_VARS_MOR_09_3_&pn
           ) by nzcon;
    disconnect from nzcon;
  quit;
 %end;
 %else %do;
  proc sql;  
    connect USING NZUSER as nzcon ;
    execute( create table &target_lib3..SCORECARD_VARS_MOR_09_3 as (
             select *
             from &target_lib3..SCORECARD_VARS_MOR_09_3_&pn
 	     ) with data;
           ) by nzcon;

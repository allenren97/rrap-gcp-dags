/**************************************************************************** 
 * Job:             J_RRAP_TL10_2403_BASEL_PNL_LN_LGDND_  A57SWEI7.BH0009IT * 
 *                  SEG_MTH_RLZ_VAL                                         * 
 ...
 *                                                                          * 
 * Source Tables:   BASEL_PNL_LN_LGD_SEG_ACCT_XREF -      A57SWEI7.BE000440 * 
 *                  NZRRAP.BASEL_PNL_LN_LGD_SEG_ACCT_XR                     * 
 *                  EF                                                      * 
 *                  PSNL_LN_LGD_OBSVT_PT_RELZ_VAL -       A57SWEI7.BE000449 * 
 *                   NZRRAP.PSNL_LN_LGD_OBSVT_PT_RELZ_VAL                   * 
 ...
 *                  BASEL_SEG - NZRRAP.BASEL_SEG          A57SWEI7.BE00041L * 
 * Target Tables:   BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL -    A57SWEI7.BE000441 * 
 ...
 *                  _VAL                                                    * 
 *                                                                          * 
 * Generated on:    Thursday, June 29, 2017 2:39:58 PM EDT                  * 
...
%let jobID = %quote(A57SWEI7.BH0009IT);
%let etls_jobName = %nrquote(J_RRAP_TL10_2403_BASEL_PNL_LN_LGDND_SEG_MTH_RLZ_VAL);
...
/*==========================================================================* 
 * Step:            Get_run_dates_for_LGDND               A57SWEI7.BK000YYJ * 
 * Transform:       Extract                                                 * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    Extract - work.TIME_KEY               A57SWEI7.BN000NIT * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YYJ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
proc sql;
   create table work.TIME_KEY as
      select
         TM_ID,
         TM_LVL_END_DT,
         (TM_ID-1440) as PRIOR36MONTHS length = 8
            format = 11.
            informat = 11.
            label = 'PRIOR36MONTHS'
   from &SYSLAST
...
/**  Step end Get_run_dates_for_LGDND **/

/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YYK * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    Extract - work.TIME_KEY               A57SWEI7.BN000NIT * 
 * Target Table:    Loop - work.WE98GJI                   A57SWEI7.BN000NIU * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YYK);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2GC5LF; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2GC5LP(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2GC5LZ; 
      
         /*==========================================================================* 
          * Step:            Delete_LGDND_records_only             A57SWEI7.BK000YYL * 
          * Transform:       Delete                                                  * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Tables:   Loop - work.WE98GJI                   A57SWEI7.BN000NIU * 
          *                  BASEL_SEG - NZRRAP.BASEL_SEG          A57SWEI7.BE00041L * 
          * Target Table:    BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL -    A57SWEI7.BE000441 * 
          *                  NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ                     * 
          *                  _VAL                                                    * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YYL);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         PROC SQL NOPRINT;
         SELECT DISTINCT BASEL_SEG_ID INTO :SEG_IDS SEPARATED BY ',' FROM NZRRAP.BASEL_SEG WHERE BASEL_SEG_NM LIKE '%LGD-ND%';
         QUIT;
         
         %let MODULE=TL;
         %let BATCH=2.4;
         
         %etl_job_start(TARGET_TABLE = NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL,
         ...
         %macro etls_sql_delete;
            ...
            %let etls_tableExist = %eval(%sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL, DATA)) or 
                  %sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL, VIEW))); 
            ...
            %do;  /* if table does not exist  */ 
               ...
               data NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL
                       (dbnull = (
                                  BASEL_SEG_ID = NO
                                  MTH_TM_ID = NO
                                  BASEL_MODEL_ID = NO
                  ...
                  attrib LGD_ACCT_TOT_RTO length = 8
                     format = 17.8
                     informat = 17.8
                     label = 'LGD_ACCT_TOT_RTO'; 
                  attrib TOT_ACCT_CNT length = 8
                     format = 11.
                     informat = 11.
                     label = 'TOT_ACCT_CNT'; 
                  attrib LGD_SEG_RTO length = 8
                     format = 17.8
                     informat = 17.8
                     label = 'LGD_SEG_RTO'; 
            ...
            proc sql;
               delete from NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL
               where
                  BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL.BASEL_SEG_ID IN (
                     select distinct
                     ...
                     where
                        BASEL_SEG_TO_DEL.BASEL_SEG_NM LIKE '%LGD-ND%'
                  )
                  and BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL.MTH_TM_ID = &DEL_MTH_TM_ID
            ;quit;
         ...
         /**  Step end Delete_LGDND_records_only **/
         
         /*==========================================================================* 
          * Step:            Inner Join_for_LGD                    A57SWEI7.BK000YYM * 
          * Transform:       Join                                                    * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Tables:   BASEL_PNL_LN_LGD_SEG_ACCT_XREF -      A57SWEI7.BE000440 * 
          ...
          *                  PSNL_LN_LGD_OBSVT_PT_RELZ_VAL -       A57SWEI7.BE000449 * 
          *                   NZRRAP.PSNL_LN_LGD_OBSVT_PT_RELZ_VAL                   * 
          * Target Table:    Join - work.WE1Q3O7                   A57SWEI7.BN000NIV * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YYM);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         
         %let etls_recnt = 0;
         %macro etls_recordCheck; 
            %let etls_recCheckExist = %eval(%sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_ACCT_XREF, DATA)) or 
                  %sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_ACCT_XREF, VIEW))); 
            
            %if (&etls_recCheckExist) %then
            %do;
               proc sql noprint;
                  select count(*) into :etls_recnt from NZRRAP.BASEL_PNL_LN_LGD_SEG_ACCT_XREF;
               quit;
            %end;
         %mend etls_recordCheck;
         %etls_recordCheck;
         ...
         proc sql;
            ...
            select
               BASEL_PNL_LN_LGD_SEG_ACCT_XREF.BASEL_SEG_ID length = 8   
                  format = 11.
                  informat = 11.
                  label = 'BASEL_SEG_ID',
               &MTH_TM_ID as MTH_TM_ID length = 8   
                  ...
                  informat = 11.
                  label = 'MTH_TM_ID',
               BASEL_PNL_LN_LGD_SEG_ACCT_XREF.BASEL_MODEL_REL_ID length = 8   
                  format = 11.
                  informat = 11.
                  label = 'BASEL_MODEL_REL_ID',
               BASEL_PNL_LN_LGD_SEG_ACCT_XREF.BASEL_MODEL_ID length = 8   
                  format = 11.
                  informat = 11.
                  label = 'BASEL_MODEL_ID',
               PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.OBSVTN_MTH_TM_ID length = 8   
                  format = 11.
                  informat = 11.
                  label = 'OBSVTN_MTH_TM_ID',
               SUM(CASE 
               WHEN (PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.LGD_NPV_ALL_COST_FLR_CAP_150_RTO NE . OR PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.LGD_NPV_ALL_COST_FLR_CAP_150_RTO IS NOT NULL) 
               THEN 1 ELSE 0 END) as MODEL_DFT_F_CNT length = 8   
                  format = 11.
                  informat = 11.
                  label = 'MODEL_DFT_F_CNT',
               SUM(PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.LGD_NPV_ALL_COST_FLR_CAP_150_RTO) as LGD_ACCT_TOT_RTO length = 8   
                  format = 30.8
                  informat = 30.8
                  label = 'LGD_ACCT_TOT_RTO',
               COUNT(BASEL_PNL_LN_LGD_SEG_ACCT_XREF.BASEL_ACCT_ID) as TOT_ACCT_CNT length = 8   
                  format = 20.
                  informat = 20.
                  label = 'TOT_ACCT_CNT'
            from
               NZRRAP.BASEL_PNL_LN_LGD_SEG_ACCT_XREF as BASEL_PNL_LN_LGD_SEG_ACCT_XREF, 
               NZRRAP.PSNL_LN_LGD_OBSVT_PT_RELZ_VAL as PSNL_LN_LGD_OBSVT_PT_RELZ_VAL
            where
               BASEL_PNL_LN_LGD_SEG_ACCT_XREF.BASEL_ACCT_ID = PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.BASEL_ACCT_ID
               and PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.LGD_TYPE = 'LGD-ND'
               and BASEL_PNL_LN_LGD_SEG_ACCT_XREF.MTH_TM_ID = &PRIOR36MONTHS
               and PSNL_LN_LGD_OBSVT_PT_RELZ_VAL.OBSVTN_MTH_TM_ID = &PRIOR36MONTHS
            group by
               BASEL_SEG_ID,
               MTH_TM_ID,
               BASEL_MODEL_REL_ID,
         ...
         /*==========================================================================* 
          * Step:            Lookup                                A57SWEI7.BK000YYO * 
          * Transform:       Lookup                                                  * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Tables:   Join - work.WE1Q3O7                   A57SWEI7.BN000NIV * 
          *                  BASEL_SEG - NZRRAP.BASEL_SEG          A57SWEI7.BE00041L * 
          *                  BASEL_SEG_RPTG_PARM -                 A57SWEI7.BE00041M * 
          *                   NZRRAP.BASEL_SEG_RPTG_PARM                             * 
          * Target Table:    Lookup - work.PRELOAD                 A57SWEI7.BN000NIW * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YYO);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         DATA  work.PRELOAD
                  (keep = BASEL_SEG_ID MTH_TM_ID BASEL_MODEL_REL_ID BASEL_MODEL_ID 
                          OBSVTN_MTH_TM_ID MODEL_DFT_F_CNT TOT_ACCT_CNT LGD_ACCT_TOT_RTO SEG_NUM 
                          BASEL_SEG_NM FINAL_RTO )
               ; 
            ...
            attrib BASEL_SEG_ID length=8 format=11. label="BASEL_SEG_ID"
                   ...
                   TOT_ACCT_CNT length=8 format=20. label="TOT_ACCT_CNT"
                   LGD_ACCT_TOT_RTO length=8 format=30.8 label="LGD_ACCT_TOT_RTO"
         ...
         /*==========================================================================* 
          * Step:            Table Loader                          A57SWEI7.BK000YYP * 
          * Transform:       Table Loader (Version 2.1)                              * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Lookup - work.PRELOAD                 A57SWEI7.BN000NIW * 
          * Target Table:    BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL -    A57SWEI7.BE000441 * 
          *                  NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ                     * 
          *                  _VAL                                                    * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YYP);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         %macro etls_loader;
               ...
               create view work.W2GC5NO as
                  select
                     ...
                     TOT_ACCT_CNT   
                        format = 11.
                        informat = 11.,
                     (CASE WHEN BASEL_SEG_NM ='DTL LGD-ND' AND SEG_NUM =8 THEN 1.06 
                        	WHEN BASEL_SEG_NM ='ITL LGD-ND' AND SEG_NUM IN (7,8) THEN 1.06 
                        	ELSE LGD_ACCT_TOT_RTO  / MODEL_DFT_F_CNT END) as LGD_SEG_RTO length = 8
                        format = 17.8
                        informat = 17.8,
                     FINAL_RTO as LGD_FINAL_RPTG_RTO   
                        format = 17.8
                        informat = 17.8,
            ...
            proc append base = NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL (BULKLOAD=YES BL_METHOD=CLILOAD)
         ...
         %let etls_recnt = 0;
         %macro etls_recordCheck; 
            %let etls_recCheckExist = %eval(%sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL, DATA)) or 
                  %sysfunc(exist(NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL, VIEW))); 
            
            %if (&etls_recCheckExist) %then
            %do;
               proc sql noprint;
                  select count(*) into :etls_recnt from NZRRAP.BASEL_PNL_LN_LGD_SEG_MTH_RLZ_VAL;
               quit;
            %end;
         %mend etls_recordCheck;
         %etls_recordCheck;
   ...
   %mend etls_processToLoopW2GC5LP;
   ...
   %let etls_parameterTable = work.W2GC5O1; 
   ...
   data &etls_statusTable 
      (keep = etls_handleName etls_machineId etls_startTime etls_endTime etls_status 
              etls_jobRC TM_ID TM_LVL_END_DT PRIOR36MONTHS
      );
   ...
   proc sql; 
      create table &etls_parameterTable as 
         select TM_ID as MTH_TM_ID, 
                ...
                PRIOR36MONTHS as PRIOR36MONTHS 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2GC5LP(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YYQ * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YYQ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

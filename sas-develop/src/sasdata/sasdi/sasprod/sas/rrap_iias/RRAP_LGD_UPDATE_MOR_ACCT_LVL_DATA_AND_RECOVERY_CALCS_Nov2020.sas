...
execute(
	delete from &FRG_DB..BNSMORT_ACCT_LVL_DATA t1
where t1.MTH_TM_ID >=&START_TM_ID AND t1.MTH_TM_ID <=&END_TM_ID)by iiascon;
...
proc sql;
	...
	select * from connection to iiascon(select t1.Mort_num,
				...
				from &RRAP_DB..BASEL_MORT_MTH_SNAPSHOT t1
					...
					where t1.MTH_TM_ID>=&START_TM_ID and t1.MTH_TM_ID<=&END_TM_ID;);
	disconnect from iiascon;
quit;
	...
	create table SCORE_SEG_ACCTS as
		SELECT *
			From connection to iiascon 
				...
				where t1.PROCESS_DATE>=&START_DATE and t1.PROCESS_DATE<=&END_DATE
				)
	;
	Disconnect from iiascon;
QUIT;

...
execute(
create table &RRAP_DB..SPL_COM_MISC_TMP1 as
(select 
      C.PIT_STATUS_V2 as PIT_STATUS, 
      sum(a.TOT_CRNT_BAL_AMT) as TOT_CRNT_BAL_AMT 
      from 
         &RRAP_DB..AIRB_RECON_APRVD_SNAPSHOT as X 
 inner join
         &RRAP_DB..BASEL_PSNL_LOAN_MTH_SNAPSHOT as A on 
         ...
         inner join 
         &RRAP_DB..BASEL_PSNL_LOAN_ACCT_DRVD_VARS as B 
         on 
         (  A.MTH_TM_ID = B.MTH_TM_ID 
            and A.BASEL_ACCT_ID = B.BASEL_ACCT_ID 
         ) 
 inner join
         &RRAP_DB..BASEL_PSNL_LOAN_ACCT_DRVD_VARS_2 as C
         on
   ...
   where 
      ...
      and A.SCRTY_CD != '99' 
group by C.PIT_STATUS_V2) WITH DATA; ) by nzcon;
...
%PUT SUM_DEF=&SUM_DEF;
%PUT TOTAL=&TOTAL;
execute (drop table &RRAP_DB..SPL_COM_MISC_TMP1) by nzcon;
...
execute(
create table &RRAP_DB..MORTGAGE_GAP_TMP1 as (
select sum(TOT_SUSP_BAL_AMT) as TOT_SUSP_BAL_AMT from &RRAP_DB..BASEL_MORT_MTH_SNAPSHOT a 
inner join &FRG_USR..&BNS_ACCTS_IN_DEF. b
on a.MORT_NUM=CAST(b.MORTGAGE_NO AS VARCHAR(1000)) 
WHERE A.MTH_TM_ID=&mth_tm_id AND A.PD_OFF_F='Y' AND A.TOT_SUSP_BAL_AMT<0) with data;
) by nzcon;
...
execute (drop table &RRAP_DB..MORTGAGE_GAP_TMP1;) by nzcon;
...
from DB2RRAP.BASEL_SEC_ADJ_FACTR_MTH_SNAP WHERE MTH_TM_ID=&mth_tm_id and secrtztn_tp_cd = 'CC';
...
SUBJECT= "[RRAP] SPL Commercial Misclassification Breakdown - &MonthYear.";
...
%MACRO SECUREMAIL2;
FILENAME OUTMAIL EMAIL
SUBJECT= "[RRAP] Mortgage Gap - &MonthYear.";
...
%MACRO SECUREMAIL3;
FILENAME OUTMAIL EMAIL
SUBJECT= "[RRAP] Total Adjustment Outstanding Balance After CC Securitization Process - &MonthYear.";

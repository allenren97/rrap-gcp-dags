...
%let etls_jobname = J_RRAP_DT4_0410_DT4_PD12_RLZ_DRVD_VARS.sas;
...
*  Target Table:  DT4_PD12_RLZ_DRVD_VARS
*  
...
%rrap_defaulter_model(
		 IIAS_LIBREF = NZUSER
		,_DEFAULTER_TABLE = &RRAP_WRK..DT4_PD12_RLZ_OBS_WINDOW
...
execute(DELETE from &RRAP_DB..DT4_PD12_RLZ_DRVD_VARS WHERE process_mth_tm_id = &mth_tm_id.;) by nzcon;
execute(commit;) by nzcon;
quit;

proc sql;
connect using NZRRAP as nzcon;
execute(INSERT INTO &RRAP_DB..DT4_PD12_RLZ_DRVD_VARS
SELECT 
	 a.SRC_SYS_CD
	,a.basel_acct_id
	,a.mort_no
...
;) by nzcon;
execute(commit;) by nzcon;
execute(CALL SYSPROC.ADMIN_CMD (%nrbquote('RUNSTATS ON TABLE &RRAP_DB..DT4_PD12_RLZ_DRVD_VARS on KEY COLUMNS and INDEXES ALL'));) by nzcon;
execute(COMMIT;) by nzcon;
quit;

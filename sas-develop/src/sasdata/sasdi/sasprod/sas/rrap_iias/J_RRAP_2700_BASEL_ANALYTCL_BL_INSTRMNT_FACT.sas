...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name : J_RRAP_2700_BASEL_ANALYTCL_BL_INSTRMNT_FACT.sas
*  Target Database: IIAS EDRTLRP1D
*  Target Table:  BASEL_ANALYTCL_BL_INSTRMNT_FACT  
*  
*  Purpose: Load the final IIAS Instrument Fact table
*
*  Frequency: Monthly
*
...
*   2023-01-10: Hadi Dimashkieh - Adding v15 formulaic
*	2023-01-13: Ganesh P: RRMSS-1907 Add 3 columns - CLP_FLAG BCAR_SCHED_NM BCAR_SCHED_NUM_50		
*	2023-01-26: Add lookup mapping for BCAR_SCHED_NUM_50, BCAR_SCHED_NM (RRMSS-1874) 	
*   2024-01-05: Nimrat G - RRMSS-2387 Change floor for CMHC from 0 to 0.0005
***************************************************************************************************************************;
...
execute(drop table &EDRTLRFRGP1D..TMP_INSTRMNT_FACT_DUP if exists; commit;)by nzcon;
execute(CREATE TABLE &EDRTLRFRGP1D..TMP_INSTRMNT_FACT_DUP AS (SELECT * FROM &EDRTLRFRGP1D..TMP_INSTRMNT_FACT WHERE basel_acct_id IN 
		(SELECT basel_acct_id FROM (SELECT basel_acct_id, ROW_NUMBER() OVER (PARTITION BY BASEL_ACCT_ID) AS rowid FROM &EDRTLRFRGP1D..TMP_INSTRMNT_FACT) a  WHERE a.rowid  <> 1)) WITH DATA; COMMIT;) by nzcon;
quit;

proc sql;
connect using nzrrap as nzcon;
execute(DELETE FROM (SELECT ROW_NUMBER() OVER (PARTITION BY BASEL_ACCT_ID) AS rowid FROM &EDRTLRFRGP1D..TMP_INSTRMNT_FACT) AS a WHERE a.rowid <> 1; COMMIT;) by nzcon;
execute(CALL SYSPROC.ADMIN_CMD (%nrbquote('RUNSTATS ON TABLE &EDRTLRFRGP1D..TMP_INSTRMNT_FACT on KEY COLUMNS and INDEXES ALL')); commit;) by nzcon;
quit;
...
CREATE TABLE FLOOR_IF_DATAPREP AS select * from connection to nzcon(
	...
	FROM &EDRTLRFRGP1D..TMP_INSTRMNT_FACT a
		LEFT JOIN &RRAP_DB..RPTG_CCF_LKP b 
		...
		LEFT JOIN &RRAP_DB..RPTG_PRD_LKP_KS l1
		...
		LEFT JOIN &RRAP_DB..RPTG_PRD_LKP_SPL l2
		...
		LEFT JOIN &RRAP_DB..RPTG_PRD_LKP_MOR l3
		...
		LEFT JOIN &RRAP_DB..BASEL_REVLVNG_CR_BASE_DRVD_VARS f
			ON a.BASEL_ACCT_ID = f.BASEL_ACCT_ID AND a.MTH_TM_ID = f.MTH_TM_ID and a.SRC_SYS_CD = 'KS'
		LEFT JOIN &RRAP_DB..RPTG_LGD_HC_LKP g
			ON case 
		...
		LEFT JOIN &RRAP_DB..BASEL_REVLVNG_CR_MTH_SNAPSHOT h
			ON a.BASEL_ACCT_ID = h.BASEL_ACCT_ID AND a.MTH_TM_ID = h.MTH_TM_ID and a.SRC_SYS_CD = 'KS' 
		LEFT JOIN &RRAP_DB..BASEL_ACCT_PRFM_FACT i
...
execute(drop table &EDRTLRFRGP1D..FLRD_INSTR_FACT if exists; commit;) by nzcon;
quit;
...
proc sql;
connect using NZRRAP as nzcon;
execute(delete from &RRAP_DB..BASEL_ANALYTCL_BL_INSTRMNT_FACT where mth_tm_id in (&mth_tm_id., &mth_tm_id. -80); commit;) by nzcon;
quit;

proc sql;
connect using NZRRAP as nzcon;
execute( insert into &RRAP_DB..BASEL_ANALYTCL_BL_INSTRMNT_FACT
select 
 a.MTH_TM_ID
,a.SRC_SYS_CD
,a.BASEL_ACCT_ID
...
FROM &EDRTLRFRGP1D..TMP_INSTRMNT_FACT a
LEFT JOIN &EDRTLRFRGP1D..FLRD_INSTR_FACT b 
ON a.MTH_TM_ID = b.MTH_TM_ID AND a.BASEL_ACCT_ID = b.BASEL_ACCT_ID 

LEFT JOIN &EDRTLRP1D..RMA_BASEL3_PRDCT_SCHD_BCAR50_LKP c /* 1500K override. RRMSS-1874*/
...
LEFT JOIN &EDRTLRP1D..RMA_BASEL3_PRDCT_SCHD_BCAR50_LKP d /*RRMSS-1874*/

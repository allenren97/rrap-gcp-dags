...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2401_PD_SEG_ACCT_XREF
*  Target Database: EDRTLRPLL
*  Target Table:  PD_SEG_ACCT_XREF
*  
...
%let jobID = %quote(A57SWEI7.BH0009I0);
%let etls_jobName = %nrquote(J_PLL_KS10_2401_PD_SEG_ACCT_XREF);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YSO * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSO);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YSP * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSP);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YSQ * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NGF * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSQ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YSR * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NGF * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGG * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSR);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F4OGC; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F4OGM(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F4OGW; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_PD_SEG_ACCT_XREF          A57SWEI7.BK000YSS * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGG * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YSS);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         %etl_job_start(TARGET_TABLE=NETCON.PD_SEG_ACCT_XREF);
         ...
         EXECUTE(DELETE FROM &net_db..PD_SEG_ACCT_XREF WHERE MTH_TM_ID=&TM_DIM_ID) BY NZCON1;
         QUIT;
         ...
         PROC SQL NOPRINT;
         	...
         	where BASEL_SEG_NM like '%PD%' and 
         	(BASEL_SEG_NM like ('%HELOC%') OR BASEL_SEG_NM like ('%CC%') OR BASEL_SEG_NM like ('%LOC%') OR BASEL_SEG_NM like ('%SL%'))
         ...
         /*	BELOW QUERY CHANGED DUE TO REQUIREMENT AS PART OF Q4 PHASE - SEP162015*/
         	...
         	Proc sql;
               CONNECT USING NETCON AS IIASCON;
               EXECUTE(
                              INSERT INTO
                              EDRTLRPLL.PD_SEG_ACCT_XREF (
                                    BASEL_ACCT_ID,
                                    MTH_TM_ID,
                                    BASEL_SEG_ID,
                                    BASEL_MODEL_REL_ID,
         ...
         /**  Step end RRAP_DM_INS_PD_SEG_ACCT_XREF **/
         
      %mend etls_jobW2F4OGW; 
   ...
   %mend etls_processToLoopW2F4OGM;
   ...
   %let etls_parameterTable = work.W2F4OGK; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F4OGM(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YST * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YST);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

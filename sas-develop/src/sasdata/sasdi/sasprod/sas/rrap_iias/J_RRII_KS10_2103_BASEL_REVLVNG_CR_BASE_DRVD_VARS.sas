/**************************************************************************** 
 * Job:             J_RRII_KS10_2103b_BASEL_REVLVNG_CR_BASE_DRVD_VARS	    * 
 *                                                                          * 
 ...
 * Changes:			2022: Basel III changes
 *					- Added 2 new columns - PIT_STAT_VER_2_CD180 
 *											PIT_STAT_VER_2_CD90
 *					- Changed logic for PIT_STAT_VER_2_CD
 *       2023-04-04 - Add exposure flag TOTAL_EXPSR_ABOVE_LMT_F (RRMSS-1712)
 *       2024-07-04 - update PIT_STAT_VER_2_CD for Step cross default (RRMSS-2840)
 ****************************************************************************/ 
...
RUN;
				      ...
				      (CASE 
						...
						(CASE 
				         ...
				         ELSE 
				         	CASE  
				               WHEN CHRG_OFF_CD ='1' THEN  'CHG' 	
				               WHEN ( BNS_DLQNT_DAY<210 and NOT (TOT_NEW_BAL_AMT> 0 AND  CHRG_OFF_CD IN ('N','Q')) ) THEN  'CUR'       
								WHEN (   TOT_NEW_BAL_AMT>0 and TOT_NEW_BAL_AMT=TOT_UNPAID_FNCL_CHRG_AMT and CHRG_OFF_CD<>'1'
                                      and  NOT( PREV_TOT_NEW_BAL_AMT>0 and  PREV_CHRG_OFF_CD IN ('N','Q' ))
                                      and PREV_TOT_NEW_BAL_AMT>0 ) THEN 'CUR'
	...
	data LD_BASEL_REVL_CR_BASE_DRVD_VARS2(index=(BASEL_ACCT_ID))
/*	(keep=MTH_TM_ID BASEL_ACCT_ID BASEL_CUST_ID ACCT_NUM SML_BUS_F STEP_CD TRNST_EXCLSN_F ACCRL_STAT_F LTV_TP_CD BNKRPY_F INSRT_PROCESS_TMSTMP UPDT_PROCESS_TMSTMP BASEL_PRD_CD BASEL_PRD_DESC CONSM_SCORECRD_EXCLSN_F CONSM_PRD_TREATMNT_CD HELOC_F PIT_STAT_VER_2_CD REVISED_EXPSR_AMT RS_F )*/
	...
	format INSRT_PROCESS_TMSTMP UPDT_PROCESS_TMSTMP datetime25.6  ACCT_NUM $50. PIT_STAT_VER_2_CD OLD_PIT_STAT_VER_2_CD $10. STEP_CD $10.
	Note1 Note2 Note3 Note4  $100.;
	
	PIT_STAT_VER_2_CD90=OLD_PIT_STAT_VER_2_CD;
	PIT_STAT_VER_2_CD180=PIT_STAT_VER_2_CD;

	if BASEL_PRD_CD='CC' AND HELOC_F = 'N' then do;
		PIT_STAT_VER_2_CD=PIT_STAT_VER_2_CD180;
		end;
	else do;
		PIT_STAT_VER_2_CD=PIT_STAT_VER_2_CD90;
		end;
...
/*		else if strip(PRD_CD) notin ('SCL', 'VIC') then do; STEP_CD= 'O'; end;*/
	...
	ELSE IF TOT_NEW_BAL_AMT<=0 and (SUBSTR(BLOCK_RECL_CD,1,1)='V' or BLOCK_RECL_CD = 'FX') THEN DO; CONSM_SCORECRD_EXCLSN_F= 'Y'; END;
	...
	ELSE IF TOT_NEW_BAL_AMT<=0 and (SUBSTR(BLOCK_RECL_CD,1,1)='V' or BLOCK_RECL_CD = 'FX') then do; CONSM_PRD_TREATMNT_CD='Z'; end;
...
/*----- RRMSS-2840 override PIT_STAT_VER_2_CD with edrtlrp1d.PIT_STATUS_PRE_STEP for Step cross default -----*/
proc sql;
	create table PIT_OVERRIDE as 
	select BASEL_ACCT_ID, CROSS_DFLT_PIT_STATUS 
      from NZRRAP.PIT_STATUS_PRE_STEP 
     where MTH_TM_ID=&TM_ID. and SRC_SYS_CD='KS' and CROSS_DFLT_PIT_OVERRIDE_F='Y'
;
	select COUNT(*) into :KS_CNT from NZRRAP.PIT_STATUS_PRE_STEP where MTH_TM_ID=&TM_ID. and SRC_SYS_CD='KS'
;
	select COUNT(*) into :KS_OVR_CNT from NZRRAP.PIT_STATUS_PRE_STEP where MTH_TM_ID=&TM_ID. and SRC_SYS_CD='KS' and CROSS_DFLT_PIT_OVERRIDE_F='Y'
;
quit;
%put NOTE: %trim(&KS_CNT) KS records were found in Pit pre-Step table for month %trim(&TM_ID);
%put NOTE: %trim(&KS_OVR_CNT) KS records were found to have Pit override for month %trim(&TM_ID);


data LD_BASEL_REVL_CR_BASE_DRVD_VARS2;
	modify LD_BASEL_REVL_CR_BASE_DRVD_VARS2 PIT_OVERRIDE(rename=(CROSS_DFLT_PIT_STATUS=PIT_STAT_VER_2_CD));
	by BASEL_ACCT_ID;
run;
/*-----------------------------------------------------------------------------------------------------------*/
...
proc sql;
	SELECT a.THRSHLD INTO :THRSHLD 
	FROM NZRRAP.RPTG_PRD_LKP_THRSHLD a
	INNER JOIN NZRRAP.TM_DIM b ON b.TM_LVL='Month' AND b.TM_ID=&MTH_TM_ID. 
		AND b.TM_LVL_END_DT BETWEEN a.EFF_FROM_YR_MTH AND a.EFF_TO_YR_MTH;

	CREATE TABLE LD_BASEL_REVL_CR_BASE_DRVD_VARS3 AS 
  	SELECT A.*
		,(CASE WHEN MISSING(B.TOT_EXPSR) THEN '' WHEN B.TOT_EXPSR>&THRSHLD. THEN 'Y' ELSE 'N' END) AS TOTAL_EXPSR_ABOVE_LMT_F  
	FROM LD_BASEL_REVL_CR_BASE_DRVD_VARS2 as A
	LEFT JOIN (SELECT BASEL_CUST_ID, BASEL_PRD_CD, SUM(REVISED_EXPSR_AMT) AS TOT_EXPSR
	              FROM LD_BASEL_REVL_CR_BASE_DRVD_VARS2
				  WHERE BASEL_CUST_ID>0 AND CONSM_PRD_TREATMNT_CD='A' AND HELOC_F = 'N' AND PIT_STAT_VER_2_CD IN ('CUR', 'DEF') 
					AND SML_BUS_F='N' AND TRNST_EXCLSN_F='N' AND STRIP(BASEL_PRD_CD) IN ('CC', 'LOC', 'SL A')  
    	          GROUP BY BASEL_CUST_ID, BASEL_PRD_CD) AS B
				ON A.BASEL_CUST_ID=B.BASEL_CUST_ID AND A.BASEL_PRD_CD=B.BASEL_PRD_CD 
					AND CONSM_PRD_TREATMNT_CD='A' AND HELOC_F = 'N' AND PIT_STAT_VER_2_CD IN ('CUR', 'DEF') 
					AND SML_BUS_F='N' AND TRNST_EXCLSN_F='N'; 


	UPDATE LD_BASEL_REVL_CR_BASE_DRVD_VARS3 SET TOTAL_EXPSR_ABOVE_LMT_F= CASE WHEN REVISED_EXPSR_AMT>&THRSHLD. THEN 'Y' ELSE 'N' END 
	WHERE BASEL_CUST_ID=-1 AND CONSM_PRD_TREATMNT_CD='A' AND HELOC_F = 'N' AND PIT_STAT_VER_2_CD IN ('CUR', 'DEF') 
		AND SML_BUS_F='N' AND TRNST_EXCLSN_F='N' AND STRIP(BASEL_PRD_CD) IN ('CC', 'LOC', 'SL A');

	UPDATE LD_BASEL_REVL_CR_BASE_DRVD_VARS3 SET TOTAL_EXPSR_ABOVE_LMT_F='N' 
	WHERE CONSM_PRD_TREATMNT_CD='A' AND (HELOC_F ='Y' OR STRIP(BASEL_PRD_CD) IN ('SL', 'SL B')) 
		AND PIT_STAT_VER_2_CD IN ('CUR', 'DEF') AND SML_BUS_F='N' AND TRNST_EXCLSN_F='N';
quit;

%put Exposure threshold is &THRSHLD.;


	data LD_BASEL_REVL_CR_BASE_DRVD_VARS
	(keep=MTH_TM_ID BASEL_ACCT_ID BASEL_CUST_ID PIT_STAT_VER_2_CD90 PIT_STAT_VER_2_CD180 ACCT_NUM SML_BUS_F STEP_CD TRNST_EXCLSN_F ACCRL_STAT_F LTV_TP_CD BNKRPY_F INSRT_PROCESS_TMSTMP UPDT_PROCESS_TMSTMP BASEL_PRD_CD BASEL_PRD_DESC CONSM_SCORECRD_EXCLSN_F CONSM_PRD_TREATMNT_CD HELOC_F PIT_STAT_VER_2_CD  REVISED_EXPSR_AMT RS_F TOTAL_EXPSR_ABOVE_LMT_F)
	;
	set LD_BASEL_REVL_CR_BASE_DRVD_VARS3;

...
libname edl_rcrr hadoop server="sdpsvrwm0163.scglobal.ad.scotiacapital.com" schema=prod_rcrr1 DBMAX_TEXT=300 
authdomain="EDL_Auth" 
...
proc sql;
	...
	select * from connection to hdpcon (
		select cast(null as decimal(11,0)) mth_tm_id,
a.mth_end_dt,
cast(a.src_sys_cd as varchar(30)) src_sys_cd,
cast(null as decimal(20,0)) basel_acct_id, 
cast(a.acct_num as varchar(80)) acct_num,
...
on a.mth_end_dt=b.mth_end_dt and a.src_sys_cd=b.src_sys_cd and a.acct_num=b.acct_num
where
a.src_sys_cd in ('KQ', 'GZ', 'SL', 'TNG_MTG', 'TNG_MCAP') 

...
data work.BASEL_CCAR_EXPSR_EXTR;
	set &DB..BASEL_CCAR_EXPSR_EXTR (rename=(PD_BAND=PDx));
...
create table &lib..BASEL_CCAR_EXPSR_EXTR as 
SELECT expsr.*,
		...
		d.secrtztn_flag
FROM work.BASEL_CCAR_EXPSR_EXTR expsr
LEFT JOIN 
...
proc sort data=&lib..BASEL_CCAR_EXPSR_EXTR;
...
%macro CCAR_FILES_GENERATE(WITHZERONET=);
	...
	%if &WITHZERONET ne %then
		%do;
			...
			data BASEL_CCAR_EXPSR_EXTR_TMP0;
				retain  &reqvars.;
				set &LIB..BASEL_CCAR_EXPSR_EXTR
			...
			;
				set BASEL_CCAR_EXPSR_EXTR_TMP0;
				array cvar 
				...
				if Legal_Entity = 'DOM-BANK-ALONE' then
					output &lib..EXPSR_&WITHZERONET.;
				else if Legal_Entity = 'DOM-SUB-DUNDEE' then
					output &lib..SUB_DUNDEE_EXPSR_&WITHZERONET.;
				else if Legal_Entity = 'DOM-SUB-TNG' then
					output &lib..SUB_TNG_EXPSR_&WITHZERONET.;
				else if Legal_Entity = 'DOM-SUB-MTCC' then
					output &lib..SUB_MTCC_EXPSR_&WITHZERONET.;
				else if Legal_Entity = 'DOM-SUB-NT' then
					output &lib..SUB_NT_EXPSR_&WITHZERONET.;
				else if Legal_Entity IN ('DOM-SUB-SMC', 'DOM-SUB-MAPLE') then
					output &lib..SUB_SMC_MAPLE_EXPSR_&WITHZERONET.;
				rename
...
%else %if &WITHZERONET = %then
	%do;
		...
		;
			...
			if Legal_Entity = 'DOM-BANK-ALONE' then
				output &lib..EXPSR_&WITHZERONET.&YYYYMMDD.;
			else if Legal_Entity = 'DOM-SUB-DUNDEE' then
				output &lib..SUB_DUNDEE_EXPSR_&WITHZERONET.&YYYYMMDD.;
			else if Legal_Entity = 'DOM-SUB-TNG' then
				output &lib..SUB_TNG_EXPSR_&WITHZERONET.&YYYYMMDD.;
			else if Legal_Entity = 'DOM-SUB-MTCC' then
				output &lib..SUB_MTCC_EXPSR_&WITHZERONET.&YYYYMMDD.;
			else if Legal_Entity = 'DOM-SUB-NT' then
				output &lib..SUB_NT_EXPSR_&WITHZERONET.&YYYYMMDD.;
			else if Legal_Entity IN ('DOM-SUB-SMC', 'DOM-SUB-MAPLE') then
				output &lib..SUB_SMC_MAPLE_EXPSR_&WITHZERONET.&YYYYMMDD.;

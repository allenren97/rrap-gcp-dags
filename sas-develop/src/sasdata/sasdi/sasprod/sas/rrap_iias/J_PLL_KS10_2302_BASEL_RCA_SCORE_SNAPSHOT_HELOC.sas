...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2302_BASEL_RCA_SCORE_SNAPSHOT_HELOC
...
%let jobID = %quote(A57SWEI7.BH0009HW);
%let etls_jobName = %nrquote(J_PLL_KS10_2302_BASEL_RCA_SCORE_SNAPSHOT_HELOC);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YS0 * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS0);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YS1 * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS1);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YS2 * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NG7 * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS2);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YS3 * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NG7 * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG8 * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS3);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F62Z9; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F62ZJ(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F62ZT; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT  A57SWEI7.BK000YS4 * 
          *                  _HELOC                                                  * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG8 * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YS4);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%EXTRACT();
 ...
 /*%SAS_DATASET_CLEANUP(LIBREF=,DATASETS=);*/
         ...
         /**  Step end RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT_HELOC **/
         
      %mend etls_jobW2F62ZT; 
   ...
   %mend etls_processToLoopW2F62ZJ;
   ...
   %let etls_parameterTable = work.W2F62ZG; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F62ZJ(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YS5 * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS5);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_2701_INSTRMNT_FACT_UPDATE
*                  
...
%let etls_jobName = %nrquote(J_PLL_2701_INSTRMNT_FACT_KS_UPDATE);
...
%macro etls_loopW2F6P5H; 
   ...
   %macro etls_processToLoopW2F6P5R(parameterTable=, row=, handleName=rmt);
      ...
      %macro etls_jobW2F6P61; 
         ...
         %etl_job_start(TARGET_TABLE=EDRTLRPLL.BASEL_ANALYTCL_BL_INSTRMNT_FACT, CONDITION=%nrbquote(BASEL_MODEL_ID IN (&MODEL_ID_DEL)) );
         /*---- End of Pre-Process Code  ----*/ 
...
%MACRO UPDATE();
    ...
    LIBNAME DBCON   DB2 DATABASE="BLUDBPRD" SCHEMA="EDRTLRP1D" authdomain="IIAS_Auth"  readbuff=10000 INSERTBUFF=10000;
    PROC SQL;
        SELECT COUNT(*) INTO: count FROM DBCON.BASEL_ANALYTCL_BL_INSTRMNT_FACT
    ...
    %if &count >0 %then %do;
        ...
        EXECUTE(DELETE FROM &net_db..BASEL_ANALYTCL_BL_INSTRMNT_FACT WHERE MTH_TM_ID=&MTH_TM_ID) BY NZCON1;
        QUIT;
 
        PROC SQL;
            CONNECT TO DB2 AS IIASCON(DATABASE=BLUDBUAT authdomain="IIAS_Auth" );
            EXECUTE(INSERT INTO EDRTLRPLL.BASEL_ANALYTCL_BL_INSTRMNT_FACT (SELECT * FROM EDRTLRP1D.BASEL_ANALYTCL_BL_INSTRMNT_FACT
                    WHERE MTH_TM_ID=&MTH_TM_ID))BY IIASCON;
        ...
        PROC SQL;
            CONNECT TO DB2 AS IIASCON(DATABASE=BLUDBUAT authdomain="IIAS_Auth" );
            EXECUTE(UPDATE EDRTLRPLL.BASEL_ANALYTCL_BL_INSTRMNT_FACT t
                SET
                t.EAD_FINAL_RPTG_RTO = subquery.FINAL_RTO,
                t.EAD_basel_seg_num = subquery.SEG_NUM,
                t.ead_basel_seg_id = subquery.basel_seg_id
                FROM
                (
                SELECT
                c.SEG_NUM,
                a.EAD_basel_seg_num,
                d.FINAL_RTO, a.ead_final_rptg_rto,
                a.basel_acct_id,
                c.basel_seg_id
                FROM edrtlrp1d.BASEL_ANALYTCL_BL_INSTRMNT_FACT a
                LEFT JOIN edrtlrpll.EAD_SEG_ACCT_XREF b
                ON a.BASEL_ACCT_ID = b.BASEL_ACCT_ID
                INNER JOIN edrtlrpll.BASEL_SEG c
                ON c.BASEL_SEG_ID = b.BASEL_SEG_ID
                INNER JOIN edrtlrpll.basel_seg_rptg_parm d
                ON d.BASEL_SEG_ID = b.BASEL_SEG_ID
                WHERE a.MTH_TM_ID =&MTH_TM_ID
                AND a.SRC_SYS_CD = 'KS'
                AND a.EAD_MODEL_NM = 'CC EAD'
                AND b.MTH_TM_ID = &MTH_TM_ID
                AND b.BASEL_MODEL_ID = 8012
                AND d.BASEL_MODEL_ID = 8012
                AND d.eff_to_dt = '9999-12-31'
                AND b.BASEL_SEG_ID in(16044,
                16040,
                16038,
                16039,
                16043)
                )subquery WHERE
                t.BASEL_ACCT_ID = subquery.BASEL_ACCT_ID
                AND t.MTH_TM_ID =&MTH_TM_ID
                AND t.SRC_SYS_CD = 'KS'
                AND t.EAD_MODEL_NM = 'CC EAD';)BY IIASCON;
            DISCONNECT FROM IIASCON;
        QUIT;
    %end;  

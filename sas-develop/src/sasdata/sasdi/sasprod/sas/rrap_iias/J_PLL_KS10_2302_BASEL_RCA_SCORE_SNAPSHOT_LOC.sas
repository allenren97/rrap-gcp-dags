...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2302_BASEL_RCA_SCORE_SNAPSHOT_LOC
*  Target Database: EDRTLRPLL
*  Target Table:  BASEL_RCA_SCORE_SNAPSHOT
*  
...
%let jobID = %quote(A57SWEI7.BH0009HX);
%let etls_jobName = %nrquote(J_PLL_KS10_2302_BASEL_RCA_SCORE_SNAPSHOT_LOC);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YS6 * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS6);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YS7 * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS7);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YS8 * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NG9 * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS8);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YS9 * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NG9 * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGA * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YS9);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F6E8Z; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F6E99(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F6E9J; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT  A57SWEI7.BK000YSA * 
          *                  _LOC                                                    * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NGA * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YSA);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         %etl_job_start(TARGET_TABLE=NETCON.BASEL_RCA_SCORE_SNAPSHOT,CONDITION=%nrbquote(BASEL_MODEL_ID IN (&MODEL_ID_DEL)) );
         /*---- End of Pre-Process Code  ----*/ 
...
%MACRO EXTRACT();
      ...
      and basel_model_id in (select basel_model_id from &net_db..BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'LOC%')) BY NZCON1;
      QUIT;
      ...
      and basel_model_id in (select basel_model_id from &net_db..BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'LOC%')
...
%EXTRACT();
          ...
          /*%SAS_DATASET_CLEANUP(LIBREF=,DATASETS=);*/
         ...
         /**  Step end RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT_LOC **/
         
      %mend etls_jobW2F6E9J; 
   ...
   %mend etls_processToLoopW2F6E99;
   ...
   %let etls_parameterTable = work.W2F6E96; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F6E99(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YSB * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YSB);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

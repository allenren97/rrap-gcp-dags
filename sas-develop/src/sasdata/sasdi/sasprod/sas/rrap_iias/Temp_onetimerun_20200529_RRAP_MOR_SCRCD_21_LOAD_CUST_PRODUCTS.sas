      ...
      rsubmit &handleName wait = no sysrputsync = yes persist = no; 
         ...
         %macro etls_jobW2HBK75; 
            ...
            /* Setup to capture return codes  */ 
                     ...
                     /*    %delete_table(sas.SCORECARD_VARS_CUST_PRODS_&year._N); 
                        proc sql;
                        	connect USING OWSTAR as dbcon;
                        	execute ( 
                        			create table frg.SCORECARD_VARS_CUST_PRODS_&year._N as 
                        	 ...
                        	 , sum(CASE WHEN e.sum_srvc_code in ('RDS','REF','REI','RIF','RSP','TFS')  and prim_cust_f='P'
                        		...
                        		END) as reginvbalp
                        				from 	
                        					...
                        					frg.SCORECARD_VARS_CUST_LIST2 as d,
                        				...
                        				group by c.cust_base_key
                                               , c.time_key
                        					) with no data in "FRG_USER_DATA_DM" compress yes) by dbcon;
                        	execute ( 
                        			insert into frg.SCORECARD_VARS_CUST_PRODS_&year._N
                        				(select c.cust_base_key
                             , c.time_key
                             , sum(CASE WHEN e.sum_srvc_code in ('CHQ','SAV')
                                    THEN 1
                        ...
                        */
                        ...
                        data SCORECARD_VARS_CUST_PRODS_&year;
                           set sas.SCORECARD_VARS_CUST_PRODS_&year._N(dbsliceparm=(all,6));
                        run;
                        
                        %delete_table(&target_lib..SCORECARD_VARS_CUST_PRODS_&year);
                        data &target_lib..SCORECARD_VARS_CUST_PRODS_&year(bulkload=yes BL_METHOD=CLILOAD);
                           set SCORECARD_VARS_CUST_PRODS_&year;
                        run;

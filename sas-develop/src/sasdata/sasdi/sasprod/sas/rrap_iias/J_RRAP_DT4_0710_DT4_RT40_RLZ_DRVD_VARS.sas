...
%let etls_jobname = J_RRAP_DT4_0710_DT4_RT40_RLZ_DRVD_VARS.sas;
...
*  Target Table:  DT4_RT40_RLZ_DRVD_VARS
*  
...
execute(delete from &RRAP_DB..DT4_RT40_RLZ_DRVD_VARS where process_mth_tm_id = &mth_tm_id.;) by nzcon;
...
proc sql;
connect using nzrrap as nzcon;
execute(INSERT into &RRAP_DB..DT4_RT40_RLZ_DRVD_VARS 
...
x.EAD_BASEL_SEG_NUM, x.EAD_BASEL_SEG_ID, x.EAD_BASEL_MODEL_ID, a.MODEL_DFT_F
,p.FINAL_RTO as EST_EAD_FINAL_RPTG_RTO
,d.EAD_FLR_CAP_200_RTO
...
ON x.EAD_BASEL_SEG_ID = p.BASEL_SEG_ID AND x.EAD_BASEL_MODEL_ID = p.BASEL_MODEL_ID AND %nrbquote('&mth_end_dt_nz') BETWEEN p.EFF_FROM_DT  AND p.EFF_TO_DT 
INNER JOIN &RRAP_DB..REVLVNG_CR_EAD_OBSVTN_PT_REALZ_VAL d 
ON a.BASEL_ACCT_ID = d.BASEL_ACCT_ID AND a.OBS_MTH_TM_ID = d.OBSVTN_MTH_TM_ID

WHERE a.OBS_MTH_TM_ID = &mth_tm_id.-12*40 AND DT4_EAD_MODEL_INCL_F = 1;) by nzcon;
execute(CALL SYSPROC.ADMIN_CMD (%nrbquote('RUNSTATS ON TABLE &RRAP_DB..DT4_RT40_RLZ_DRVD_VARS on KEY COLUMNS and INDEXES ALL'));) by nzcon;
execute(COMMIT;) by nzcon;
quit;

...
proc sql ;
   ...
   from NZRRAP.CUST_MODEL ; *where time_key = 17876;				/*Sud* where time_key = 17876; */

  	select max(popn_dt) as max_date into :iwd_cust 
   from NZRRAP.IWD_CUST ; *where popn_dt = '15May2019'd;         /*Sud* Where POPN_DT = 15May2019 or 15Apr2019*/

	select max(POPN_DT) as max_date into :IWD_PD_PLN 
   from NZRRAP.IWD_PD_PLN ;*where POPN_DT = '15May2015'd;       /*Sud* Where POPN_DT = 15May2015 or 15Apr2015*/
  
   select max(time_key) as max_date into :IWF_CUST_ACTY_RLP
   from NZRRAP.IWF_CUST_ACTY_RLP ;* where time_key = 17876;      /*Sud* where time_key = 17876; */

	select max(time_lvl_end_dt) as max_date into :iwd_time
   from NZRRAP.iwd_time ;*where *time_lvl_end_dt = '31May2019'd;  /*Sud* Where time_lvl_end_dt=  '31May2019'd or 31Oct2021; */

   select max(time_key) as max_date into :IWF_CUST_ACCT
   from NZRRAP.IWF_CUST_ACCT ;*where time_key = 17876;          /*Sud* where time_key = 17876; */

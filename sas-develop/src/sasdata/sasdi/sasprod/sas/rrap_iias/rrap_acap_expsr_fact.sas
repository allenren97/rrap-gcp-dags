...
%LET TGT00=EDRRAPT.BASEL_ACAP_EXPSR_FACT;

proc sql NOPRINT;
delete from EDRRAPT.BASEL_ACAP_EXPSR_FACT  where  MTH_TM_ID= &MTH_TM_ID;
quit;
...
/*when a.SRC_SYS_CD in ('SPL') then .*/
				...
				,DLGD_RPTG_RTO as WGHTD_DLGD_RTO, DLGD_F
/*Hadi*/
...
/*			and (ADJUSTED_OS_BAL_AMT <> 0 or UNDRAWN_AMT <> 0)*/  
			...
			and DLGD_F='N'
			AND (not missing(pd_basel_seg_num) and not missing(lgd_basel_seg_num));
	quit;
%mend;

/*STEP01 - Extract current month data;*/
%getRRAPreportingExtractFact(indata=&input00.,outdata=ACAPTEMP.RRAP_Rpt_Extract00);

/*STEP02 - Split extract by source system code;*/
data ACAPTEMP.RRAP_Rpt_Extract_KS 
	ACAPTEMP.RRAP_Rpt_Extract_MOR 
	ACAPTEMP.RRAP_Rpt_Extract_SPL 
	ACAPTEMP.RRAP_Rpt_Extract_TNG ;
	set ACAPTEMP.RRAP_Rpt_Extract00;
...
/*end;*/
	*** Split into 3 output files;
	if SRC_SYS_CD in ('KS') then output ACAPTEMP.RRAP_Rpt_Extract_KS;
	else if SRC_SYS_CD in ('MOR') then output ACAPTEMP.RRAP_Rpt_Extract_MOR;
	else if SRC_SYS_CD in ('SPL') then output ACAPTEMP.RRAP_Rpt_Extract_SPL;
	else if SRC_SYS_CD in ('TNG-MOR') then output ACAPTEMP.RRAP_Rpt_Extract_TNG;
	else delete;
run;
...
%macro getRRAPreportingExtract(SRC_SYS_CD=, indata=, NCRdata=, outdata=);
	...
	proc sql noprint;
		create table ACAPTEMP._temp_aggr_base as
			select 	PD_BAND,
...
%else %if &SRC_SYS_CD=SPL %then
	...
	case 
		when mean(PD_FINAL_RPTG_RTO) >=1 then max(LGD_FINAL_RPTG_RTO) 
		else . 
	end 														
...
%else
	...
	from &indata.
	where DLGD_F='N'
		group by PD_BAND, CCAR_BASEL_PRD_TP_NM, TRNST_NUM, PRD_ID, MTH_TM_ID, SRC_SYS_CD, 
			LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, PD_BASEL_SEG_NUM, 
			PD_BASEL_SEG_ID, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID ,INSURER_FLAG
			/*hadi*/
	order by PD_BAND, CCAR_BASEL_PRD_TP_NM, TRNST_NUM, PRD_ID, MTH_TM_ID, SRC_SYS_CD, 
		LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, PD_BASEL_SEG_NUM, 
		PD_BASEL_SEG_ID, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID ,INSURER_FLAG
		/*hadi*/
	;
...
proc sql noprint;
create table ACAPTEMP._temp_aggr_NCR_balance as
select 	MTH_TM_ID,
...
from ACAPTEMP.RRAP_Rpt_Extract00
...
proc sql noprint;
create table ACAPTEMP._temp_dir_NCR as
select	distinct	
MTH_TM_ID, 
NCR_EXPSR_CL_KEY,
CR_LOSS_ALWNC_AMT
...
proc sql noprint;
create table ACAPTEMP._temp_acct_balance as
...
from ACAPTEMP.RRAP_Rpt_Extract00
where PD_FINAL_RPTG_RTO >=1
order by MTH_TM_ID, SRC_SYS_CD,CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID, 
...
proc sql noprint;
create table ACAPTEMP._temp_acct_prorated_loss as
select 	a.*, 
...
from ACAPTEMP._temp_acct_balance as a 
left join ACAPTEMP._temp_aggr_NCR_balance as b 
	on a.MTH_TM_ID=b.MTH_TM_ID and a.NCR_EXPSR_CL_KEY_VAL=b.NCR_EXPSR_CL_KEY_VAL 
left join ACAPTEMP._temp_dir_NCR as c
...
proc sql noprint;
create table ACAPTEMP._temp_aggr_ACL as
select  MTH_TM_ID,
SRC_SYS_CD,
CCAR_BASEL_PRD_TP_NM,
PD_BAND,
...
PD_BASEL_SEG_NUM, 
EAD_BASEL_SEG_NUM, 
LGD_BASEL_SEG_NUM,INSURER_FLAG,
sum(PRORATED_CR_LOSS_ALLOW_AMT) as ACL,
sum(PRORATED_CR_LOSS_ALLOW_AMT_AFTER) as After_Netting_ACL 
from ACAPTEMP._temp_acct_prorated_loss
group by MTH_TM_ID,SRC_SYS_CD, CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID, INSURER_FLAG, 
...
from ACAPTEMP._temp_aggr_base as a 
left join ACAPTEMP._temp_aggr_ACL as b
	...
	and a.TRNST_NUM=b.TRNST_NUM and a.PRD_ID=b.PRD_ID 
...
%getRRAPreportingExtract(SRC_SYS_CD=KS,
	indata=ACAPTEMP.RRAP_Rpt_Extract_KS,
	NCRdata=EDRRAPT.BASEL_CCAR_BUS_AGGRTD_FACT,
	outdata=ACAPTEMP.RRAP_Rpt_Extract_KS_Final);
%getRRAPreportingExtract(SRC_SYS_CD=MOR,
	indata=ACAPTEMP.RRAP_Rpt_Extract_MOR,
	NCRdata=EDRRAPT.BASEL_CCAR_BUS_AGGRTD_FACT,
	outdata=ACAPTEMP.RRAP_Rpt_Extract_MOR_Final);
%getRRAPreportingExtract(SRC_SYS_CD=SPL,
	indata=ACAPTEMP.RRAP_Rpt_Extract_SPL,
	NCRdata=EDRRAPT.BASEL_CCAR_BUS_AGGRTD_FACT,
	outdata=ACAPTEMP.RRAP_Rpt_Extract_SPL_Final);

/*STEP04 - Concatenate output files for KS MOR SPL;*/
%getRRAPreportingExtract(SRC_SYS_CD=TNG,
	indata=ACAPTEMP.RRAP_Rpt_Extract_TNG,
	NCRdata=EDRRAPT.BASEL_CCAR_BUS_AGGRTD_FACT,
	outdata=ACAPTEMP.RRAP_Rpt_Extract_TNG_Final);


data ACAPTEMP.RRAP_Rpt_Extract_AllBank ;
format legal_entity $40. crncy_cd $10. UNCONDTNLY_CNCLBL $10.;
	set ACAPTEMP.RRAP_Rpt_Extract_KS_Final
		ACAPTEMP.RRAP_Rpt_Extract_MOR_Final
		ACAPTEMP.RRAP_Rpt_Extract_SPL_Final
		ACAPTEMP.RRAP_Rpt_Extract_TNG_Final;
		drop src_sys_cd;
run;

proc sort data= ACAPTEMP.RRAP_Rpt_Extract_AllBank nodupkey; 
by 
MTH_TM_ID LEGAL_ENTITY CCAR_BASEL_PRD_TP_NM PD_BAND TRNST_NUM PRD_ID LGD_BASEL_SEG_NUM EAD_BASEL_SEG_NUM INSUR_F; 
quit;


PROC SQL NOPRINT;
INSERT INTO EDRRAPT.BASEL_ACAP_EXPSR_FACT  
(
LEGAL_ENTITY
,PD_BAND
,TRNST_NUM
...
,WGHTD_DLGD_RTO
FROM ACAPTEMP.RRAP_Rpt_Extract_AllBank;
QUIT;
...
/*STEP01 - Extract current month data;*/
%getRRAPreportingExtractFact_Sub(indata=&input00.,outdata=ACAPTEMP.RRAP_Rpt_Sub_Extract00);
...
from &indata.
where DLGD_F='N'
group by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID,
 		 LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID,PD_BASEL_SEG_NUM,
		PD_BASEL_SEG_ID,INSURER_FLAG


order by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID,
 		 LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID
...
proc sql noprint;
create table ACAPTEMP._temp_aggr_balance as
select 	MTH_TM_ID,
		LEGAL_ENTITY,
...
/*Get ACL Lookup data;*/
proc sql noprint;
    create table ACAPTEMP._temp_GL_ACL_balance as 
...
select 	MTH_TM_ID,
		...
		CONS_DFT_MTH_CNT,
		ACCT_NUM, INSURER_FLAG
from &indata.  
where PD_FINAL_RPTG_RTO >=1
order by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM,
		PD_BAND, TRNST_NUM, PRD_ID, LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, ACCT_NUM;
quit;
...
as PRORATED_CR_LOSS_ALLOW_AMT_after
from 	ACAPTEMP._temp_acct_balance as a 
left join ACAPTEMP._temp_aggr_balance as b on a.MTH_TM_ID=b.MTH_TM_ID and 
a.LEGAL_ENTITY=b.LEGAL_ENTITY 
left join ACAPTEMP._temp_GL_ACL_balance as c on a.MTH_TM_ID=c.MTH_TM_ID and 
...
select  MTH_TM_ID,
		...
		sum(PRORATED_CR_LOSS_ALLOW_AMT_after) as After_Netting_ACL 
from 	ACAPTEMP._temp_acct_prorated_loss
group by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM, INSURER_FLAG,
		PD_BAND, TRNST_NUM, PRD_ID, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_NUM
order by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM,
		PD_BAND, TRNST_NUM, PRD_ID, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_NUM
		;
quit;
...
%getRRAPreportingExtractSubs(indata=ACAPTEMP.RRAP_Rpt_Sub_Extract00,
							 ACLlookup=EDRRAPT.BASEL_SUBSIDIARY_ACL_LKP, 
							 outdata=ACAPTEMP.RRAP_Rpt_Extract_Subsidiary);

proc sort data= ACAPTEMP.RRAP_Rpt_Extract_Subsidiary nodupkey; 
by 
MTH_TM_ID LEGAL_ENTITY CCAR_BASEL_PRD_TP_NM PD_BAND TRNST_NUM PRD_ID LGD_BASEL_SEG_NUM EAD_BASEL_SEG_NUM INSUR_F; 
quit;
...
,TRIM(INSUR_F)
,WGHTD_DLGD_RTO
FROM ACAPTEMP.RRAP_Rpt_Extract_Subsidiary;
QUIT;


/*UPDATE NULL VALUES TO 0 FOR TNG PRODUCTS*/
PROC SQL NOPRINT;
UPDATE EDRRAPT.BASEL_ACAP_EXPSR_FACT 
...
%else
	...
	where DLGD_F='Y'
		group by PD_BAND, CCAR_BASEL_PRD_TP_NM, TRNST_NUM, PRD_ID, MTH_TM_ID, SRC_SYS_CD, 
			LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, PD_BASEL_SEG_NUM, 
			PD_BASEL_SEG_ID, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID ,INSURER_FLAG, WGHTD_DLGD_RTO
			/*hadi*/
	order by PD_BAND, CCAR_BASEL_PRD_TP_NM, TRNST_NUM, PRD_ID, MTH_TM_ID, SRC_SYS_CD, 
		LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, PD_BASEL_SEG_NUM, 
		PD_BASEL_SEG_ID, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID ,INSURER_FLAG, WGHTD_DLGD_RTO
...
proc sql noprint;
	create table ACAPTEMP._temp_aggr_ACL as
		select  MTH_TM_ID,
			...
			PD_BASEL_SEG_NUM, 
			EAD_BASEL_SEG_NUM, 
			LGD_BASEL_SEG_NUM,INSURER_FLAG,WGHTD_DLGD_RTO,
			sum(PRORATED_CR_LOSS_ALLOW_AMT) as ACL,
			sum(PRORATED_CR_LOSS_ALLOW_AMT_AFTER) as After_Netting_ACL 
		from ACAPTEMP._temp_acct_prorated_loss
			group by MTH_TM_ID,SRC_SYS_CD, CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID, INSURER_FLAG, WGHTD_DLGD_RTO,
				PD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_NUM
...
proc sort data= ACAPTEMP.RRAP_Rpt_Extract_AllBank nodupkey; 
by 
MTH_TM_ID LEGAL_ENTITY CCAR_BASEL_PRD_TP_NM PD_BAND TRNST_NUM PRD_ID LGD_BASEL_SEG_NUM EAD_BASEL_SEG_NUM INSUR_F WGHTD_DLGD_RTO; 
quit;
...
group by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM, PD_BAND, TRNST_NUM, PRD_ID,
 		 LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, LGD_BASEL_SEG_ID, EAD_BASEL_SEG_ID,PD_BASEL_SEG_NUM,
		PD_BASEL_SEG_ID,INSURER_FLAG, WGHTD_DLGD_RTO
...
select 	MTH_TM_ID,
		...
		CONS_DFT_MTH_CNT,
		ACCT_NUM, INSURER_FLAG, WGHTD_DLGD_RTO
from &indata.  
where PD_FINAL_RPTG_RTO >=1
order by MTH_TM_ID, LEGAL_ENTITY, CCAR_BASEL_PRD_TP_NM,
		PD_BAND, TRNST_NUM, PRD_ID, LGD_BASEL_SEG_NUM, EAD_BASEL_SEG_NUM, ACCT_NUM;

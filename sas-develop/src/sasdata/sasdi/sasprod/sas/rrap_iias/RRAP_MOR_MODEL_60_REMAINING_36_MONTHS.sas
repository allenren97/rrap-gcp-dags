/**************************************************************************** 
 * Job:             RRAP_MOR_MODEL_60_REMAINING_36_MONTHS A57SWEI7.BH000KPO * 
 * Description:                                                             * 
 *                                                                          * 
 ...
 *                                                                          * 
 * Generated on:    Wednesday, March 28, 2018 3:48:44 PM EDT                * 
...
%let jobID = %quote(A57SWEI7.BH000KPO);
%let etls_jobName = %nrquote(RRAP_MOR_MODEL_60_REMAINING_36_MONTHS);
...
/*==========================================================================* 
 * Step:            Set Dates                             A57SWEI7.BK000ZGY * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Target Table:    User Written - work.W1J0WMO           A57SWEI7.BN000NSG * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000ZGY);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%let _OUTPUT_path = %nrquote(/User Written_A57SWEI7.BN000NSG%(WorkTable%));
%let _OUTPUT_type = 1;
...
%let _OUTPUT1_path = %nrquote(/User Written_A57SWEI7.BN000NSG%(WorkTable%));
%let _OUTPUT1_type = 1;
...
%dt;
proc sort; by run_date; run;


data _null_;
	x=put(intnx('month',"&end_period_dt."d,-36,'e'),date9.);
...
if mths > 1 then do;
   do i=1 to mths;
      if i=1 then 
         run_date="&start_date"d;
      else 
         run_date=intnx('month',"&start_date"d,i-1,'e');
      _run_date_yymth = put(run_date,yymmn.);
      _run_date = put(run_date,yymmdd10.);
      output;
   end;
end;
else if mths = 1 then do;
   run_date="&start_date"d;
   _run_date_yymth = put(run_date,yymmn.);
   _run_date = put(run_date,yymmdd10.); 
   output;
...
data _output;
merge _output_all(in=a) _output_initial(in=b);
by run_date;
if a and not b;
run;
...
data _null_; 
        ...
        call symputx('mth_del'!!n, mth_tm_id); 
        call symputx('nobs', _n_); 
run;   

%do i = 1 %to &nobs.; 
        %put Cleaning up: &&mth_del&i.; 
        proc sql; 
                connect using nzuser as nzcon; 
                execute(delete from &frg_db..basel_analytcl_bl_inst_fct_BNS where mth_tm_id=&&mth_del&i.) by nzcon; 
        quit; 
%end; 
...
/*proc sql;*/
/*connect using nzuser as nzcon;*/
/*execute (truncate table basel_analytcl_bl_inst_fct_BNS ) by nzcon;*/
/*quit;*/
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000ZGZ * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.W1J0WMO           A57SWEI7.BN000NSG * 
 * Target Table:    Loop - work.W2B6FZD                   A57SWEI7.BN000NSH * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000ZGZ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW3VWVRJ; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW3VWVRT(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
   ...
   %mend etls_processToLoopW3VWVRT;
   ...
   %let etls_parameterTable = work.W3VWW9C; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW3VWVRT(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000ZH1 * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000ZH1);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

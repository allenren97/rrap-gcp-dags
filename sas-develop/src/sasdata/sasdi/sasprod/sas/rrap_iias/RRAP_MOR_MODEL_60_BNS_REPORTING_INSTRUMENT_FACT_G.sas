options errorabend;
/*********************************************************************************** 
 * Job:             RRAP_MOR_MODEL_60_BNS_REPORTING_INSTRUMENT_FACT_G              *	 
 * Description:                                                             	   * 
 * Generated on:    Wednesday, September 26, 2018 2:32:05 PM EDT            	   * 
 * Generated by:    owprdsas                                                	   * 
 * Changes: 		2022 - Basel III Changes
					1) Add new elements 		
  					  RNTL_PRPTY_F,CURRENCY_MISMATCH_F,TOT_EXPSR_ABOVE_1500K_LMT_F 
					 ,ORIG_AMT_LOAN,LTV_BUCKET, TRANSACTOR_F, PD_90_day_F
					2) Add new columns for PMI - Uninsured LGD, DLGD
 *																				   *
 ...
 * 					Nov 2022: Transactor flags (RRMSS-1590/1770) 				   *
...
%let etls_jobName = %nrquote(RRAP_MOR_MODEL_60_BNS_REPORTING_INSTRUMENT_FACT_G);
...
%let transformID = %quote(A57SWEI7.BK0011VH);
%let trans_rc = 0;
...
%let transformID = %quote(A57SWEI7.BK0011VI);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

%let _INPUT_count = 0;
%let _OUTPUT_count = 1;
%let _OUTPUT = work.W1J0WMO;
%let _OUTPUT_connect = ;
%let _OUTPUT_engine = ;
%let _OUTPUT_memtype = DATA;
%let _OUTPUT_options = %nrquote();
%let _OUTPUT_alter = %nrquote();
%let _OUTPUT_path = %nrquote(/User Written_A57SWEI7.BN000Q5J%(WorkTable%));
%let _OUTPUT_type = 1;
...
%let _OUTPUT_col0_table = work.W1J0WMO;
...
%let _OUTPUT_col1_table = work.W1J0WMO;
...
%let _OUTPUT_col2_table = work.W1J0WMO;
...
%let _OUTPUT1_col0_table = work.W1J0WMO;
...
%let _OUTPUT1_col1_table = work.W1J0WMO;
...
%let _OUTPUT1_col2_table = work.W1J0WMO;
...
execute (truncate table &FRG_DB..basel_analytcl_bl_inst_fct_BNS IMMEDIATE) by nzcon;
quit;
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK0011VJ * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.W1J0WMO           A57SWEI7.BN000Q5J * 
 * Target Table:    Loop - work.W2B6FZD                   A57SWEI7.BN000Q5K * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK0011VJ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_recordCheck; 
   %let etls_recCheckExist = %eval(%sysfunc(exist(work.W1J0WMO, DATA)) or 
         %sysfunc(exist(work.W1J0WMO, VIEW))); 
   ...
   %do;
      ...
      proc contents data = work.W1J0WMO out = work.etls_contents(keep = nobs) noprint; 
      run; 
...
%let SYSLAST = %nrquote(work.W1J0WMO); 
...
%macro etls_loopWH1WNCP; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopWH1WNCZ(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobWH1WND9; 
      
         /**************************************************************************** 
          * Job:             RRAP_MOR_MODEL_60_BNS_REPORTING_INST  A57SWEI7.BH000LHP * 
          *                  RUMENT_FACT                                             * 
          * Description:                                                             * 
          *                                                                          * 
         ...
         /* General macro variables  */ 
         %let jobID = %quote(A57SWEI7.BH000LHP);
         %let etls_jobName = %nrquote(RRAP_MOR_MODEL_60_BNS_REPORTING_INSTRUMENT_FACT);
         %let etls_userID = %nrquote(owprdsas);
         ...
         /*==========================================================================* 
          * Step:            AM_rrap_mor_bns_autoexec              A57SWEI7.BK0011XC * 
          * Transform:       AM_rrap_mor_bns_autoexec                                * 
          * Description:     BNS Mortgage jobs imported from IW503 leverage this     * 
          *                   transformation                                         * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK0011XC);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
		...
		proc datasets lib=work;
         ...
         proc sql;
         	...
         	create table work.BASEL_ACCT_PRFM_FACT as 
         		select * from connection to nzcon (	
         		select A.* from &EDRTLRP1D..BASEL_ACCT_PRFM_FACT A 					
         ...
         proc sql;
         	create table work.base_with_brdm  as
         		...
         		case 
         			...
         			and a.total_suspense < 0 
         			then (-1*a.total_suspense)
         	...
         	coalesce(c.ncr_ltv_key_val, '0405') as ncr_ltv_key_val,
               e.score_foreclose_ind,
               e.score_unemp_ratio,
               e.score_d2dbalmax12m,
               e.score_month_def,
               e.score_index_teranetv,
               e.score_ltv,
               e.bns_lgd_d_segment
         	from work.base_bns_mor_rpt_tbl as a 
               ...
               left join
                     NZUSER.BNS_LGD_D_SCORED_SEG_ACCTS as e
                     on (a.mortgage_no=e.mortgage_no and
                     put(datepart(a.PROCESS_DATE),yymmdd10.) = put(e.TIME_KEY,yymmdd10.))
         ...
         data work.with_scores;
            ...
            do;
                ...
                * Changed: 03FEB2025 - Now using scores calculated in GCP
                *-----------------------------------------------*/ /*
               ...
               /* Changed: 03FEB2025 - Now using segment calculated in GCP */
...
/*END Add UNINSURED_LGD_SEG_NUM */
         ...
         proc sql noprint;
            insert into nzuser.BASEL_ANALYTCL_BL_INST_FCT_BNS (bulkload=yes BL_METHOD=CLILOAD)
            select a.mth_tm_id,
                  a.src_sys_cd,
                  a.basel_acct_id,
                  a.unq_acct_id,
         ...
         /*----------------------------------------------------
          * 7. Load Work Table with all Variables to Netezza  
          *----------------------------------------------------*/
         
         %macro load_all_ratios(_run_Date_yymth=);
         
         data _null_;
             call symput('filedate',put(today(),date7.));
         run;
         
         %put filedate = &filedate; 
         
         %delete_table(nzuser.BNS_WITH_ALL_RTOS_&_run_Date_yymth._&filedate.);
         
         proc sql;
            create table nzuser.BNS_WITH_ALL_RTOS_&_run_Date_yymth._&filedate. /*(bulkload=yes)*/ as
            select a.*
         /*   from intmed.bns_with_all_ratios_&_run_date_yymth as a;*/
         ...
         /* (bulkload=yes bl_options="ctrlchars true nullvalue ' ' ")*/
         %load_all_ratios(_run_Date_yymth=&_run_date_yymth);
         ...
         /**  Job end RRAP_MOR_MODEL_60_BNS_REPORTING_INSTRUMENT_FACT **/
         
         /* Reset jobid and name */
         %let jobID = %quote(A57SWEI7.BH000LHV);
         %let etls_jobName = %nrquote(RRAP_MOR_MODEL_60_BNS_REPORTING_INSTRUMENT_FACT_G);
         
      %mend etls_jobWH1WND9; 
   ...
   %mend etls_processToLoopWH1WNCZ;
   ...
   %let etls_parameterTable = work.WH1WNJX; 
   %let &etls_controlName = 0;
   %let etls_controlTable = work.W1J0WMO;
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopWH1WNCZ(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK0011VL * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK0011VL);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
PROC SQL NOPRINT;
CONNECT USING NZUSER AS NZCON;
EXECUTE(DELETE FROM &FRG_DB..BASEL_ANLYT_BL_INST_FCT_BNS_DLGD WHERE MTH_TM_ID=&MTH_TM_ID) BY NZCON;
DISCONNECT FROM NZCON;
QUIT;

/* STEP TO VERIFY SOURCE DATA*/
proc sql ;	
     select count(*) INTO: manul_error_check from NZRRAP.DLGD_SCRI_QTR_DRVD_VARS WHERE MTH_TM_ID=&CURR_QTR_MTH_TM_ID;
QUIT;
...
CONNECT USING NZUSER AS NZCON;
EXECUTE(
INSERT INTO &FRG_DB..BASEL_ANLYT_BL_INST_FCT_BNS_DLGD 
SELECT 
MTH_TM_ID
,SRC_SYS_CD
,BASEL_ACCT_ID
				...
				END AS DELTA_P
					...
					FROM (
							...
							FROM (
									...
									FROM 
									&FRG_DB..BASEL_ANALYTCL_BL_INST_FCT_BNS A,&RRAP_DB..BASEL_MORT_MTH_SNAPSHOT B 
									WHERE 
									TRIM(A.MORT_NUM)=TRIM(B.MORT_NUM) AND A.MTH_TM_ID=&MTH_TM_ID AND B.MTH_TM_ID=&MTH_TM_ID
					) SUBQ1
						LEFT OUTER JOIN &FRG_DB..LTV_VAR_CLUS_LTV_FINAL_CMA FIR ON TRIM(SUBQ1.MORT_NUM)=FIR.MORTGAGE_NO AND FIR.YYMTH=&YEARMONTH
						...
						LEFT OUTER JOIN (SELECT DISTINCT MTH_TM_ID, LABEL_2, INDEX FROM &RRAP_DB..TERANET_HOUSE_PRC_INDEX_CMA WHERE MTH_TM_ID=&MTH_TM_ID) INDX 
							ON UPPER(TRIM(INDX.LABEL_2)) =UPPER(TRIM(SUBQ2.METROPOLITAN))
						LEFT OUTER JOIN (SELECT DISTINCT MTH_TM_ID, LABEL_2, INDEX FROM &RRAP_DB..TERANET_HOUSE_PRC_INDEX_CMA WHERE MTH_TM_ID=&MTH_TM_ID-1440) INDX12 

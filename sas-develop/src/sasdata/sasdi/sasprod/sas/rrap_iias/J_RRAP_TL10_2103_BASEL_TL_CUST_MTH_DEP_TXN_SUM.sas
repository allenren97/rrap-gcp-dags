         		...
         		proc sql;
         		    ...
         		    select * from connection to dbcon (
         				select c.cust_base_key,
         		                          ...
         		                          from &RRAP_DB..iwf_acty_rollup as a,
         		                               &RRAP_DB..iwf_cust_acct as c,
         		                               &RRAP_DB..iwd_chnl as d
...
%put &=sqlrc;
         		...
         		proc sql;
         		    ...
         		    select * from connection to dbcon (
         				select cust_base_key, CUST_ID from &rrap_db..cust_xref group by cust_base_key, CUST_ID
         				);
         		quit;
...
%put &=sqlrc;
         		...
         		%let dt = %SYSFUNC(datetime(),best18.);
                     ...
                     CREATE TABLE BASEL_TL_CUST_MTH_DEP_TXN_SUM AS
                     select  a.TIME_KEY AS MTH_TM_ID FORMAT=BEST.,
                                 c.BASEL_CUST_ID FORMAT=BEST.,
                                 CUST_NB_TRNX_DEP AS DEP_CNT FORMAT=BEST.,
                                 CUST_NB_TRNX_NSF AS NSF_CNT FORMAT=BEST.,
                                 CUST_OD_USAGE AS OVDR_CNT FORMAT=BEST., 
                                 CUST_NB_TRNX_PAYROLL AS PAYRL_CNT FORMAT=BEST.,
                                 CUST_TOT_TRNX_DEP AS TOT_DEP_AMT FORMAT=COMMA17.3, 
                                 CUST_TOT_TRNX_NSF AS TOT_NSF_AMT FORMAT=COMMA17.3,
                                 CUST_TOT_TRNX_PAYROLL AS TOT_PAYRL_AMT FORMAT=COMMA17.3, 
                                 CUST_TOT_TRNX_WD AS TOT_WTHDRWL_AMT FORMAT=COMMA17.3, 
                                 CUST_NB_TRNX_WD AS WTHDRWL_CNT FORMAT=BEST.,
                                 &dt AS INSRT_PROCESS_TMSTMP FORMAT=DATETIME.
...
PROC APPEND BASE=NZRRAP.BASEL_TL_CUST_MTH_DEP_TXN_SUM(BULKLOAD=YES BL_METHOD=CLILOAD)
            DATA=BASEL_TL_CUST_MTH_DEP_TXN_SUM FORCE;
            
/*proc sql;
   select count(*) into: manul_error_check from NZRRAP.BASEL_TL_CUST_MTH_DEP_TXN_SUM where mth_tm_id=&mth_tm_id.;
quit;

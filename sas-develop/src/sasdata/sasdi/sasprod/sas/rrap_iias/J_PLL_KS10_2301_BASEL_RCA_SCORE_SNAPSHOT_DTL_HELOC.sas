...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2301_BASEL_RCA_SCORE_SNAPSHOT_DTL_HELOC
*  Target Database: EDRTLRPLL
*  Target Table:  BASEL_RCA_SCORE_DTL_SNAPSHOT_HC
*  
...
%let jobID = %quote(A57SWEI7.BH0009HT);
%let etls_jobName = %nrquote(J_PLL_KS10_2301_BASEL_RCA_SCORE_SNAPSHOT_DTL_HELOC);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YRI * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRI);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YRJ * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRJ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YRK * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NG1 * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRK);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YRL * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NG1 * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG2 * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRL);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F7LQ6; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F7LQG(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F7LQQ; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT  A57SWEI7.BK000YRM * 
          *                  _DTL_HELOC                                              * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG2 * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YRM);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         PROC SQL noprint;
         select basel_model_id INTO :MODEL_ID_DEL SEPARATED BY ',' from NETCON.BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'HELOC%';
         QUIT;
         %let MODEL_ID_DEL=&MODEL_ID_DEL.;
         
         %etl_job_start(TARGET_TABLE=NETCON.BASEL_RCA_SCORE_DTL_SNAPSHOT_HC,CONDITION=%nrbquote(BASEL_MODEL_ID IN (&MODEL_ID_DEL)) );
...
%MACRO EXTRACT();
   ...
   DATA prg_data.CRITERIAS_DTL_HELOC;
   ...
   INSERT INTO prg_data.CRITERIAS_HELOC
   SELECT
   ...
   AND HDR.BASEL_MODEL_ID in (select basel_model_id from NETCON.BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'HELOC%'); 
   ...
   SELECT COUNT(*) INTO :CRITERIA_COUNT FROM prg_data.CRITERIAS_HELOC;

   %DO i=1 %TO &CRITERIA_COUNT;

   /*FOR EVERY VALUE OF i, SELECT THE SUBQUERY INTO A SAS VARIABLE*/
   SELECT BIN_CRTRIA_SQL_CD_STRG INTO :VAR1 FROM prg_data.CRITERIAS_HELOC WHERE PRIM_ID=&i;

   /*INSERT ROW BY ROW INTO THE PREDEFINED SAS TABLE THE ACCOUNTS FOR EVERY CRITERIA*/
   INSERT INTO prg_data.CRITERIAS_DTL_HELOC (BASEL_MODEL_SCORECRD_DTL_ID,MTH_TM_ID,BASEL_ACCT_ID,BASEL_MODEL_ID,BASEL_MODEL_SCORECRD_HDR_ID,BIN,PT_CNT,INSRT_PROCESS_TMSTMP)
   ...
   AND D.BASEL_MODEL_SCORECRD_HDR_ID=(SELECT BASEL_MODEL_SCORECRD_HDR_ID FROM prg_data.CRITERIAS_HELOC WHERE PRIM_ID=&i) 
   AND D.BASEL_MODEL_SCORECRD_DTL_ID=(SELECT BASEL_MODEL_SCORECRD_DTL_ID FROM prg_data.CRITERIAS_HELOC WHERE PRIM_ID=&i)
   /*AND TRIM(D.CRTRIA_DESC)=(SELECT CRTRIA_DESC FROM prg_data.CRITERIAS_HELOC WHERE PRIM_ID=&i) */
   AND D.BIN=(SELECT BIN FROM prg_data.CRITERIAS_HELOC WHERE PRIM_ID=&i);
...
PROC APPEND BASE=NETCON.BASEL_RCA_SCORE_DTL_SNAPSHOT_HC ( BULKLOAD=YES BL_METHOD=CLILOAD ) DATA=prg_data.CRITERIAS_DTL_HELOC FORCE; RUN;
...
/*         PT_CNT,*/
/*         INSRT_PROCESS_TMSTMP*/
/*         from prg_data.CRITERIAS_DTL_HELOC*/
/*         ;*/
/*         quit;*/
         
         
         %SAS_DATASET_CLEANUP(LIBREF=PRG_DATA,DATASETS=CRITERIAS_HELOC CRITERIAS_DTL_HELOC);
         ...
         /**  Step end RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT_DTL_HELOC **/
         
      %mend etls_jobW2F7LQQ; 
   ...
   %mend etls_processToLoopW2F7LQG;
   ...
   %let etls_parameterTable = work.W2F7LQE; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F7LQG(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YRN * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRN);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

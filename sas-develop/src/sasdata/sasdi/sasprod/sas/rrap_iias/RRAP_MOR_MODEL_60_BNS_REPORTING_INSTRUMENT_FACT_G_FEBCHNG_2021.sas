/**************************************************************************** 
 * Job:             RRAP_MOR_MODEL_60_BNS_REPORTING_INST  A57SWEI7.BH000LHV * 
 *                  RUMENT_FACT_G                                           * 
 * Description:                                                             * 
 *                                                                          * 
...
/*%let start_period_dt=28FEB2017;*/
/*%let end_period_dt=28FEB2017;*/
...
*execute (truncate table &FRG_DB..basel_analytcl_bl_inst_fct_BNS ) by nzcon;
...
%macro etls_loopWH1WNCP; 
   ...
   %macro etls_processToLoopWH1WNCZ(parameterTable=, row=, handleName=rmt);
      ...
      %macro etls_jobWH1WND9; 
         ...
         /*   create table bns_with_all_ratios (drop = pd_band_exposure_class_key_value) as*/
            select a.*,
...
/*,DELTA_P AS PRPTY_VAL_CORR_PCTG --> Sudhir
...
,CASE 
	  WHEN SUBQ4.DLGD_F='Y' AND SUBQ4.SCRTY_TP_DESC='Uninsured' and SUBQ4.ADDON IS NOT NULL THEN 
			...
			AS DLGD_RPTG_RTO
/*,DLGD_FINAL_RPTG_RTO AS DLGD_RPTG_RTO --> Sudhir*/
,LTV_PERCENTAGE
		...
		FROM
			...
			SELECT SUBQ2.*, INDX.INDEX, INDX12.INDEX AS INDEX_12_QTR_AGO, (SUBQ2.INDEX_TERANETV/INDX.INDEX)*INDX12.INDEX AS HPV_12_QTR_AGO, SCRI.METRPL_BREACH_F,
				CASE 
					...
					WHEN SUBQ2.DLGD_F<>'N' AND SCRI.METRPL_BREACH_F='Y' THEN MAX(1-(((SUBQ2.INDEX_TERANETV/INDX.INDEX)*INDX12.INDEX)/NULLIF(SUBQ2.INDEX_TERANETV,0)),0.25) 
					WHEN SUBQ2.DLGD_F<>'N' AND SCRI.METRPL_BREACH_F<>'Y' THEN MAX(1-(((SUBQ2.INDEX_TERANETV/INDX.INDEX)*INDX12.INDEX)/NULLIF(SUBQ2.INDEX_TERANETV,0)),0) 
				END AS DELTA_P
			/*	CASE 
...
/*			AS DLGD_FINAL_RPTG_RTO*/
		/*	CASE 
				WHEN SUBQ2.DLGD_F='Y' AND SUBQ2.SCRTY_TP_DESC='Uninsured' and ADDON IS NOT NULL THEN 
						MAX(0.1, SUBQ2.LGD_FINAL_RPTG_RTO, MIN(1, (SUBQ2.lgd_unadjusted_rptg_rto + ADDON))) 
				WHEN SUBQ2.DLGD_F='Y' AND SUBQ2.SCRTY_TP_DESC='Insured' and ADDON IS NOT NULL THEN 
						MAX(0.1, SUBQ2.LGD_FINAL_RPTG_RTO, MIN(1, (SUBQ2.lgd_insured_rptg_rto + ADDON)))
				ELSE SUBQ2.LGD_FINAL_RPTG_RTO 
			END /*DLGD Insured changes effective November 2017 -- Modified September 2018 */
		/*	AS DLGD_FINAL_RPTG_RTO */

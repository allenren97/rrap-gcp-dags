/**************************************************************************** 
 * Job:             J_rrap_exception_report_3_TL_Segment  A57SWEI7.BH000AEK * 
 *                  ation_Distribution                                      * 
 * Description:                                                             * 
 *                                                                          * 
 ...
 *                                                                          * 
 * Generated on:    Thursday, March 22, 2018 2:03:20 PM EDT                 * 
...
%let jobID = %quote(A57SWEI7.BH000AEK);
%let etls_jobName = %nrquote(J_rrap_exception_report_3_TL_Segmentation_Distribution);
...
/*==========================================================================* 
 * Step:            rrap_exception_report_autoexec        A57SWEI7.BK0011WT * 
 * Transform:       AM_rrap_exception_report_autoexec                       * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK0011WT);
%let trans_rc = 0;
...
/* Sudhir --> This code does not work in IIAS */ /* 
PROC SQL;
...
%let file_name_tl =TL_PLL_Segmentation_Distribution_&file_name_date..xls;
...
from &BNS_PREV_TABLE
where CONSM_PRD_TREATMNT_CD ="A" and SML_BUS_F = "N" and PIT_STAT_CD="CUR"
and TRNST_EXCLSN_F = "N" and LGD_MODEL_NM="BNS MOR LGD-ND" 
union all
select &current_ym as YRMTH,
...
line "TL Parallel Run Segmentation Distribution &file_name_date - SI above threshold";
endcomp;
run;
...
endcomp;
compute before _PAGE_;
line "TL Parallel Run Segmentation Distribution &file_name_date";
endcomp;
run;
proc report data=work.legend split='~' missing  noheader
;
...
SUBJECT= "[RRAP] SI above threshold for TL Parallel Run Segmentation Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&si_email_list);
     put;
...
%else %do;
FILENAME OUTMAIL EMAIL ATTACH=("&file_path.&file_name_tl")
SUBJECT= "[RRAP] TL Parallel Run Segmentation Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&email_list);

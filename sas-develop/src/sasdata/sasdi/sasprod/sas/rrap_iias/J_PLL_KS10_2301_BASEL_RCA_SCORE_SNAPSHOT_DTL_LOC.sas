...
***************************************************************************************************************************;
***************************************************************************************************************************;
*  Job Name:J_PLL_KS10_2301_BASEL_RCA_SCORE_SNAPSHOT_DTL_LOC
*  Target Database: EDRTLRPLL
*  Target Table:  BASEL_RCA_SCORE_DTL_SNAPSHOT_LOC
*  
...
%let jobID = %quote(A57SWEI7.BH0009HU);
%let etls_jobName = %nrquote(J_PLL_KS10_2301_BASEL_RCA_SCORE_SNAPSHOT_DTL_LOC);
...
/*==========================================================================* 
 * Step:            AM_rrap_ks_autoexec                   A57SWEI7.BK000YRO * 
 * Transform:       AM_rrap_ks_autoexec                                     * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRO);
%let trans_rc = 0;
...
/*==========================================================================* 
 * Step:            AM_sas_dataset_cleanup                A57SWEI7.BK000YRP * 
 * Transform:       AM_sas_dataset_cleanup                                  * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRP);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            List of Time ID's                     A57SWEI7.BK000YRQ * 
 * Transform:       User Written                                            * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    TM_DIM - NZRRAP.TM_DIM                A57SWEI7.BE000412 * 
 * Target Table:    User Written - work.time_key          A57SWEI7.BN000NG3 * 
 *                                                                          * 
 * User Written:    SourceCode                                              * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRQ);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
/*==========================================================================* 
 * Step:            Loop                                  A57SWEI7.BK000YRR * 
 * Transform:       Loop                                                    * 
 * Description:                                                             * 
 *                                                                          * 
 * Source Table:    User Written - work.time_key          A57SWEI7.BN000NG3 * 
 * Target Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG4 * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRR);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
...
%macro etls_loopW2F7B82; 
   %local etls_filePrefix; 
   %let etls_filePrefix = ; 
   
   %macro etls_processToLoopW2F7B8C(parameterTable=, row=, handleName=rmt);
      %local etls_parmvars; 
      ...
      %macro etls_jobW2F7B8M; 
      
         /*==========================================================================* 
          * Step:            RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT  A57SWEI7.BK000YRS * 
          *                  _DTL_LOC                                                * 
          * Transform:       User Written                                            * 
          * Description:                                                             * 
          *                                                                          * 
          * Source Table:    Loop - work.WOLVMA                    A57SWEI7.BN000NG4 * 
          *                                                                          * 
          * User Written:    SourceCode                                              * 
          *==========================================================================*/ 
         
         %let transformID = %quote(A57SWEI7.BK000YRS);
         %let trans_rc = 0;
         %let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 
         ...
         PROC SQL noprint;
         select basel_model_id INTO :MODEL_ID_DEL SEPARATED BY ',' from NETCON.BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'LOC%';
         QUIT;
         %let MODEL_ID_DEL=&MODEL_ID_DEL.;
         
         %etl_job_start(TARGET_TABLE=NETCON.BASEL_RCA_SCORE_DTL_SNAPSHOT_LOC,CONDITION=%nrbquote(BASEL_MODEL_ID IN (&MODEL_ID_DEL)) );
         /*---- End of Pre-Process Code  ----*/ 
         ...
         DATA prg_data.CRITERIAS_DTL_LOC;
         ...
         INSERT INTO prg_data.CRITERIAS_LOC
         SELECT
         ...
         AND HDR.BASEL_MODEL_ID in (select basel_model_id from NETCON.BASEL_MODEL_SCORECRD_HDR where BASEL_SCORECRD_NM like 'LOC%'); 
         ...
         SELECT COUNT(*) INTO :CRITERIA_COUNT FROM prg_data.CRITERIAS_LOC;
         
         %DO i=1 %TO &CRITERIA_COUNT;
         
         /*FOR EVERY VALUE OF i, SELECT THE SUBQUERY INTO A SAS VARIABLE*/
         SELECT BIN_CRTRIA_SQL_CD_STRG INTO :VAR1 FROM prg_data.CRITERIAS_LOC WHERE PRIM_ID=&i;
         
         /*INSERT ROW BY ROW INTO THE PREDEFINED SAS TABLE THE ACCOUNTS FOR EVERY CRITERIA*/
         INSERT INTO prg_data.CRITERIAS_DTL_LOC (BASEL_MODEL_SCORECRD_DTL_ID,MTH_TM_ID,BASEL_ACCT_ID,BASEL_MODEL_ID,BASEL_MODEL_SCORECRD_HDR_ID,BIN,PT_CNT,INSRT_PROCESS_TMSTMP)
         ...
         FROM NETCON.BASEL_MODEL_SCORECRD_HDR C, NETCON.BASEL_MODEL_SCORECRD_DTL D, (SELECT * FROM CONNECTION TO DBCON (&VAR1)) X
         WHERE C.BASEL_MODEL_SCORECRD_HDR_ID=D.BASEL_MODEL_SCORECRD_HDR_ID
         AND D.BASEL_MODEL_SCORECRD_HDR_ID=(SELECT BASEL_MODEL_SCORECRD_HDR_ID FROM prg_data.CRITERIAS_LOC WHERE PRIM_ID=&i) 
         AND D.BASEL_MODEL_SCORECRD_DTL_ID=(SELECT BASEL_MODEL_SCORECRD_DTL_ID FROM prg_data.CRITERIAS_LOC WHERE PRIM_ID=&i)
         /*AND TRIM(D.CRTRIA_DESC)=(SELECT CRTRIA_DESC FROM prg_data.CRITERIAS_LOC WHERE PRIM_ID=&i) */
         AND D.BIN=(SELECT BIN FROM prg_data.CRITERIAS_LOC WHERE PRIM_ID=&i);
		 ...
		 PROC APPEND BASE=NETCON.BASEL_RCA_SCORE_DTL_SNAPSHOT_LOC ( BULKLOAD=YES BL_METHOD=CLILOAD ) DATA=prg_data.CRITERIAS_DTL_LOC FORCE; RUN;
...
/*         PT_CNT,*/
/*         INSRT_PROCESS_TMSTMP*/
/*         from prg_data.CRITERIAS_DTL_LOC*/
/*         ;*/
/*         quit;*/
         
         
         %SAS_DATASET_CLEANUP(LIBREF=PRG_DATA,DATASETS=CRITERIAS_LOC CRITERIAS_DTL_LOC);
         ...
         /**  Step end RRAP_DM_INS_BASEL_RCA_SCORE_SNAPSHOT_DTL_LOC **/
         
      %mend etls_jobW2F7B8M; 
   ...
   %mend etls_processToLoopW2F7B8C;
   ...
   %let etls_parameterTable = work.W2F7B8A; 
   ...
   %do; 
      %do %until (&&&etls_controlName ge &&&etls_controlName._max); 
         ...
         %etls_processToLoopW2F7B8C(parameterTable=&etls_parameterTable, row=&&&etls_controlName,
...
/*==========================================================================* 
 * Step:            Loop End                              A57SWEI7.BK000YRT * 
 * Transform:       Loop End                                                * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK000YRT);
%let trans_rc = 0;
%let etls_stepStartTime = %sysfunc(datetime(), datetime20.); 

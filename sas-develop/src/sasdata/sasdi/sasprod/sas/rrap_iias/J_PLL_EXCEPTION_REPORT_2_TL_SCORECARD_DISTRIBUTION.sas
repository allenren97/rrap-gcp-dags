/**************************************************************************** 
 * Job:             J_rrap_exception_report_2_TL_Scoreca  A57SWEI7.BH000AEH * 
 *                  rd_Distribution                                         * 
 * Description:                                                             * 
 *                                                                          * 
 ...
 *                                                                          * 
 * Generated on:    Thursday, March 22, 2018 2:03:12 PM EDT                 * 
...
%let jobID = %quote(A57SWEI7.BH000AEH);
%let etls_jobName = %nrquote(J_rrap_exception_report_2_TL_Scorecard_Distribution);
...
/*==========================================================================* 
 * Step:            rrap_exception_report_autoexec        A57SWEI7.BK0011WN * 
 * Transform:       AM_rrap_exception_report_autoexec                       * 
 * Description:                                                             * 
 *==========================================================================*/ 

%let transformID = %quote(A57SWEI7.BK0011WN);
%let trans_rc = 0;
...
%let file_name_tl=TL_PLL_Scorecard_Distribution_&file_name_date..xls;
...
AND c.basel_model_id NE 8028 /*THIS IS TO REMOVE DTL LGD-ND BASED ON RRMSS-3188*/
order by MODEL_NM
;
quit;
...
%symdel COUNT /x nowarn;
%symdel TYPE  /x nowarn;
%symdel List  /x nowarn;
...
%macro create_report;
Ods tagsets.ExcelXP path="&file_path" file="&file_name_tl" options(sheet_interval='none'); run;

%if &email_alert = 1 %then %do;
proc report data=work.spl_psi_final split='~'
...
compute after MODEL_NM;
 ...
 if (_c7_) > &Threshold then call define('_C8_',"style","style={background=red}");
 endcomp;

compute before _PAGE_;
line "TL Parallel Run Scorecard Distribution &file_name_date - SI above threshold";
endcomp;
run;
...
compute before _PAGE_;
line "TL Parallel Run Scorecard Distribution &file_name_date";
endcomp;
run;
proc report data=work.legend split='~' missing  noheader;
column BASEL_SCORECRD_NM;
...
%macro sendemail;
%if &email_alert = 1 %then %do;
FILENAME OUTMAIL EMAIL ATTACH=("&file_path.&file_name_tl")
SUBJECT= "[RRAP] SI above threshold for TL Parallel Run Scorecard Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&si_email_list);
     put;
...
%else %do;
FILENAME OUTMAIL EMAIL ATTACH=("&file_path.&file_name_tl")
SUBJECT= "[RRAP] TL Parallel Run Scorecard Distribution Report - &MonthYear.";
DATA _NULL_;
     FILE OUTMAIL
     TO= (&email_list);
...
%PUT RRAP Exception report 2 TL - Start Time: %Sysfunc(PutN(&datetime_start.,datetime18.));

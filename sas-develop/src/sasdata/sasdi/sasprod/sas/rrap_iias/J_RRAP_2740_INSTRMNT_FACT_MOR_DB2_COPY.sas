...
PROC SQL NOPRINT;
     ...
     EXECUTE(DELETE FROM &DBSCHEMA..BASEL_ANALYTCL_BL_INSTRMNT_FACT WHERE MTH_TM_ID=&MTH_TM_ID. and rtrim(ltrim(SRC_SYS_CD)) in ('MOR')) BY NZCON;
QUIT;

proc append base=DB2RRAP.BASEL_ANALYTCL_BL_INSTRMNT_FACT  (BULKLOAD=YES BL_METHOD=CLILOAD )
	data=NZRRAP.BASEL_ANALYTCL_BL_INSTRMNT_FACT(where=(mth_tm_id = &mth_tm_id. and SRC_SYS_CD = 'MOR')) force ; run;

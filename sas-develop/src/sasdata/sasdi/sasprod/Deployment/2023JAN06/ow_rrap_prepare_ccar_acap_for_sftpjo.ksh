...
#
# Script Name  : ow_rrap_prepare_ccar_acap_for_sftpjo.ksh
#                (base is ow_common_sftp.ksh)
#
# Function     : Prepare CCAR-ACAP combined files for SFTPJO by renaming files as per trancodes given from SFTPJO.
#				 NOTE:- Trancodes : JOAACCA3 (for .csv ) and JOAACCA4 ( for .ctl) files
#                
# Parameter    : $1 (Mandatory): Directory path of source (ccar) files 
#				 Note- A "/" is not required at the end of directory path
#                example: ow_rrap_prepare_ccar_for_sftpjo.ksh $OWFTP/cmf/outgoing/CCAR_ACAP $OWFTP/cmf/outgoing/jofiles/CCAR_ACAP
#
...
f_usage() {
   ...
   print "$SCRIPTNAME.ksh ccar_folder_fullpath jofiles_folder_fullpath" | tee -a ${LOGFILE}
   print " " | tee -a ${LOGFILE}
...
### Read every DR*.CSV filename and copy to "jofiles" directory by changing to correct trancode
	...
	do
		NEWFILE=$(basename "$FILE" .csv).JOAACCA3
...
### Read every DR*.CTL filename and copy to "jofiles" directory by changing to correct trancode
	...
	do
		NEWFILE=$(basename "$FILE" .ctl).JOAACCA4
		echo FILE": " $FILE " "NEWFILE: " " $NEWFILE
		cp $FILE ${jofiles_path}/$NEWFILE
		   STATUS=$?
		   if [[ $STATUS -gt 0 ]]

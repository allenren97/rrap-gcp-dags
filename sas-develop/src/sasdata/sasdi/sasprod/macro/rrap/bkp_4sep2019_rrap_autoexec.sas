...
%macro rrap_ncrbtt_autoexec();
		...
		%let FRG_DB=FRG_USER_DATA;
			...
			libname tngdata netezza server=cs2iwntzp01 database=TNGSTP1D       AUTHDOMAIN=nz_auth access=readonly;
			libname nz_frg  netezza server=cs2iwntzp01 database=frg_user_data  AUTHDOMAIN=nz_auth access=readonly; 
			libname NZRRAP  netezza server=cs2iwntzp01 database=EDRTLRP1D      authdomain=nz_auth access=readonly;
			libname NZWRKBT netezza server=cs2iwntzp01 database=EDRTLRBTWRKP1D authdomain=nz_auth;

		%if %sysfunc(substr(&etls_jobname.,15,2)) eq TL %then %do;
			libname nzuser  netezza server=cs2iwntzp01 database=EDRTLRBTWRKP1D AUTHDOMAIN=nz_auth;
		%end;
		%else %do;
			libname nzuser  netezza server=cs2iwntzp01 database=frg_user_data AUTHDOMAIN=nz_auth access=readonly;
		%end;
...
/*data _null_;abort;run;*/
%GLOBAL FRG_DB frg_lib RRAP_DB TNG_DB;
%let FRG_DB=FRG_USER_DATA;
%let frg_lib=nzuser; %let RRAP_DB=EDRTLRP1D; %let TNG_DB=TNGSTP1D;
...
libname NZ netezza server=cs2iwntzp01 database=credit_risk authdomain=nz_auth;* bulkunload=yes;
libname NZUSER netezza server=cs2iwntzp01 database=frg_user_data bulkunload=NO authdomain="NZ_AUTH"; *user=&nzuser. password="&nzpassword";
libname NZRRAP netezza server=cs2iwntzp01 database=EDRTLRP1D authdomain="NZ_AUTH";
...
LIBNAME NETCON NETEZZA SERVER=&net_serv DATABASE=&net_db authdomain=nz_auth;
LIBNAME NZRRAP NETEZZA SERVER=&net_serv DATABASE=&net_db authdomain=nz_auth;
/*NZRRAP added since the control table uses it*/
	
%LET NETE_PASS_THROUGH=SERVER=&net_serv DATABASE=&net_db authdomain=nz_auth;
LIBNAME NZUSER NETEZZA SERVER=&net_serv DATABASE=&net_wrk authdomain=nz_auth;
...
LIBNAME OWSTAR DB2 DATASRC=&DB2PASSTHRU SCHEMA=OWSTAR authdomain=db2_auth;

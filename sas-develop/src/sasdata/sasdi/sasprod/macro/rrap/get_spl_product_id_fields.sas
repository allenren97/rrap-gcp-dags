...
execute(
   ...
   modify column (prd VARCHAR(60));

   alter table &FRG_DB...&table
   modify column (sub_product VARCHAR(60));

   alter table &FRG_DB...&table
   modify column (prd_id VARCHAR(10));

   alter table &FRG_DB...&table
   modify column (prd_tp VARCHAR(10));
...
execute(
 ...
 execute (generate statistics on &FRG_DB...&table) by nzcon;
 quit;

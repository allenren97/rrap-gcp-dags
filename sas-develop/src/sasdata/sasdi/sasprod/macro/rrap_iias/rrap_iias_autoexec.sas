...
%macro rrap_dlgd_autoexec();
	...
	%global mth_tm_id mth_end_dt mth_end_dt_nz start_period_dt FRG_USR EDRTLRP1D RRAP_DB EDRTLRFRGP1D yrmth yrmth_prev;
	%LET IIASDB=BLUDBPRD; 
...
/*   %let start_period_dt=31OCT2016;*/
	 ...
	 %include "&rrap_dir./macro/rrap_iias/DT4_ETL_SCD2_LOAD.sas";
...
/* commented for connection to new gcve sql db, RRMSS-3956
...
*/
libname AIRBSQL sqlsvr dsn=AIRB_RECON_PRD SCHEMA=dbo authdomain=RRAPSQL readbuff=10000;
...
%global rrap_dir FRG_DB FRG_LIB RRAP_DB EDRLGD EDRTLRP1D EDRTLRFRGP1D;
...
/* IIAS LIB */
LIBNAME IIASLGD DB2 DATABASE="&IIASDB" SCHEMA="&EDRLGD" authdomain="IIAS_Auth" readbuff=10000 INSERTBUFF=10000;
...
/*Compensation Control*/
%global threshold; %let threshold=0.0001;
/* 10-Oct-2024 : Changes in DMS caused failure in MO_BNSTG. Confirmed that RRAP does not fetch data from DMS, hence commenting - Nikhil */
...
/* INITIALIIZING NEW SQL SERVER LIBRARIES - Added on 12DEC2021 by VK*/
/* commented for connection to new gcve sql db, RRMSS-3956
%let dsns=BASEL_DATAFEEDS_PROD;
%let sschma=dbo;
%let usrsq=usr_web_apps;
%let passwsq={SAS002}F4CDBF013EAAE2D657A0B7DD00D051C413C4275C;
libname sqlnew sqlsvr dsn=&dsns schema=&sschma. user=&usrsq. password= "&passwsq."; 
*/
libname sqlnew sqlsvr dsn=BASEL_DATAFEEDS_PRD SCHEMA=dbo authdomain=RRAPSQL readbuff=10000;
...
%macro rrap_dt4_autoexec(ENV=PROD,FREQ=QUARTER);
options errorabend mprint source validvarname=any;

%include "&rrap_dir./macro/rrap_iias/DT4_ETL_SCD2_LOAD.sas";
%include "&rrap_dir./macro/rrap_iias/rrap_defaulter_model.sas";

%global IIASDB DB2DB RRAP_DB MOR_DB RRAP_DB2 RRAP_WRK TNG_DB RRAP_LGD 
	MTH_TM_ID MTH_END_DT MTH_END_DT MTH_ST_DT MTH_END_DT_NZ YRMTH YRMTH_PREV owftp;
...
%let RRAP_DB = EDRTLRP1D;
%let RRAP_LGD = EDRLGD;
%let MOR_DB =  FRG_USER_DATA;
%let RRAP_DB2= EDRRAPT;
%let RRAP_WRK= EDRTLRFRGP1D;
%let TNG_DB  = TNGSTP1D;
****************************/;
%end;

%else %if &ENV. eq UAT %then %do;
/**** UAT SERVER ***************;
%let owftp  =  /owftp;
********************************;
%let IIASDB =  BLUDBUAT;
%let DB2DB  =  DM1P1D;
********************************;
%let RRAP_DB = EDRTLRD3D;
%let MOR_DB =  FRG_USER_DATA_3;
%let RRAP_DB2= EDRRAPT;
%let RRAP_WRK= EDRTLRWRKD3D;
%let TNG_DB  = TNGSTD3D;
********************************/
%end;

%else %if &ENV. eq DEV %then %do;
/**** INTERIM DEVELOPMENT *******;
%let owftp  =  /owftp;
********************************;
%let IIASDB =  BLUDBPRD;
%let DB2DB  =  DM1P1D;
********************************;
%let RRAP_DB = TNGSTD3D;
%let RRAP_LGD = EDRLGD;
%let MOR_DB =  TNGSTD3D;
%let RRAP_DB2= EDRRAPT;
%let RRAP_WRK= TNGSTD3D;
%let TNG_DB  = TNGSTD3D;
********************************/;
%end;


libname DB2RRAP db2 database="&DB2DB."   schema=EDRRAPT     authdomain=db2_auth read_isolation_level=ur readbuff=32000;
libname NZRRAP  db2 database="&IIASDB." schema=&RRAP_DB.  authdomain=iias_auth read_isolation_level=ur readbuff=32000 insertbuff=32000;
libname NZFRG   db2 database="&IIASDB." schema=&MOR_DB.   authdomain=iias_auth read_isolation_level=ur readbuff=32000 insertbuff=32000;
libname NZUSER  db2 database="&IIASDB." schema=&RRAP_WRK. authdomain=iias_auth read_isolation_level=ur readbuff=32000 insertbuff=32000;
libname NZ_LGD  db2 database="&IIASDB." schema=&RRAP_LGD. authdomain=iias_auth read_isolation_level=ur readbuff=32000 insertbuff=32000;



%let rundate = &sysdate9.;
/*%let rundate=06SEP2023;*/

data rundate;
	format mth_end_dt date9.;
	%if &FREQ. EQ QUARTER %then %do;
		mth_end_dt= intnx('YEAR',intnx('QTR.2',"&rundate."d,-1,'e'),0,'s');
	%end;
	%else %if &FREQ. EQ MONTH %then %do;
...
proc sql;
	...
	select tm_id as MTH_TM_ID, mth_end_dt, TM_LVL_ST_DT
...
data _null_;
	...
	call symputx('mth_st_dt',put(TM_LVL_ST_DT,yymmdd10.));
...
options nosource ;
%PUT ************************************************************************************;
%PUT RUN ON &RUNDATE. at &SYSTIME. for &FREQ. ENDING &MTH_END_DT.;
%PUT MTH_TM_ID = &MTH_TM_ID.;
%PUT MTH_END_DT = &MTH_END_DT.;
%PUT MTH_ST_DT = &MTH_ST_DT.;
%PUT MTH_END_DT_NZ = &MTH_END_DT_NZ.;
%PUT YRMTH = &YRMTH.;
%PUT ************************************************************************************;
options source;

%mend rrap_dt4_autoexec;
